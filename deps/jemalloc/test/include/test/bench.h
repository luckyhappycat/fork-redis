...
static inline void fmt_nsecs(uint64_t usec, uint64_t iters, char *buf) {
    ...
    malloc_snprintf(buf, FMT_NSECS_BUF_SIZE, "%" FMTu64 ".%03" FMTu64, intpart, fracpart);
}

static inline void compare_funcs(uint64_t nwarmup, uint64_t niter, const char *name_a, void(*func_a), const char *name_b, void(*func_b)) {
