...
static extent_hooks_t hooks = {extent_alloc_hook,      extent_dalloc_hook,       extent_destroy_hook, extent_commit_hook, extent_decommit_hook,
                               extent_purge_lazy_hook, extent_purge_forced_hook, extent_split_hook,   extent_merge_hook};
...
static void *extent_alloc_hook(
    ...
    expect_ptr_eq(extent_hooks->alloc, extent_alloc_hook, "Wrong hook function");
    called_alloc = true;
    if (!try_alloc) {
        return NULL;
    }
...
static bool extent_dalloc_hook(extent_hooks_t *extent_hooks, void *addr, size_t size, bool committed, unsigned arena_ind) {
    ...
    expect_ptr_eq(extent_hooks, &hooks, "extent_hooks should be same as pointer used to set hooks");
    expect_ptr_eq(extent_hooks->dalloc, extent_dalloc_hook, "Wrong hook function");
    called_dalloc = true;
    if (!try_dalloc) {
        return true;
    }
...
static void extent_destroy_hook(extent_hooks_t *extent_hooks, void *addr, size_t size, bool committed, unsigned arena_ind) {
    ...
    expect_ptr_eq(extent_hooks, &hooks, "extent_hooks should be same as pointer used to set hooks");
    expect_ptr_eq(extent_hooks->destroy, extent_destroy_hook, "Wrong hook function");
    called_destroy = true;
    if (!try_destroy) {
        return;
    }
...
static bool extent_commit_hook(extent_hooks_t *extent_hooks, void *addr, size_t size, size_t offset, size_t length, unsigned arena_ind) {
    ...
    expect_ptr_eq(extent_hooks, &hooks, "extent_hooks should be same as pointer used to set hooks");
    expect_ptr_eq(extent_hooks->commit, extent_commit_hook, "Wrong hook function");
    called_commit = true;
    if (!try_commit) {
        return true;
    }
...
static bool extent_decommit_hook(extent_hooks_t *extent_hooks, void *addr, size_t size, size_t offset, size_t length, unsigned arena_ind) {
    ...
    expect_ptr_eq(extent_hooks, &hooks, "extent_hooks should be same as pointer used to set hooks");
    expect_ptr_eq(extent_hooks->decommit, extent_decommit_hook, "Wrong hook function");
...
static bool extent_purge_lazy_hook(extent_hooks_t *extent_hooks, void *addr, size_t size, size_t offset, size_t length, unsigned arena_ind) {
    ...
    expect_ptr_eq(extent_hooks, &hooks, "extent_hooks should be same as pointer used to set hooks");
    expect_ptr_eq(extent_hooks->purge_lazy, extent_purge_lazy_hook, "Wrong hook function");
...
static bool extent_purge_forced_hook(extent_hooks_t *extent_hooks, void *addr, size_t size, size_t offset, size_t length, unsigned arena_ind) {
    ...
    expect_ptr_eq(extent_hooks, &hooks, "extent_hooks should be same as pointer used to set hooks");
    expect_ptr_eq(extent_hooks->purge_forced, extent_purge_forced_hook, "Wrong hook function");
...
static bool extent_split_hook(
    ...
    expect_ptr_eq(extent_hooks, &hooks, "extent_hooks should be same as pointer used to set hooks");
    expect_ptr_eq(extent_hooks->split, extent_split_hook, "Wrong hook function");
    called_split = true;
    if (!try_split) {
        return true;
    }
...
static bool extent_merge_hook(
    ...
    expect_ptr_eq(extent_hooks, &hooks, "extent_hooks should be same as pointer used to set hooks");
    expect_ptr_eq(extent_hooks->merge, extent_merge_hook, "Wrong hook function");
    expect_ptr_eq((void *)((uintptr_t)addr_a + size_a), addr_b, "Extents not mergeable");
    called_merge = true;
    if (!try_merge) {
        return true;
    }
...
static void extent_hooks_prep(void) {
    ...
    expect_d_eq(mallctl("arena.0.extent_hooks", (void *)&default_hooks, &sz, NULL, 0), 0, "Unexpected mallctl() error");
}
