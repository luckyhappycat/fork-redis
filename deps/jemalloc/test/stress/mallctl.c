...
static void mallctl_short(void) {
    const char *version;
...
static void mallctlbymib_short(void) {
...
TEST_BEGIN(test_mallctl_vs_mallctlbymib_short) {
    ...
    compare_funcs(10 * 1000 * 1000, 10 * 1000 * 1000, "mallctl_short", mallctl_short, "mallctlbymib_short", mallctlbymib_short);
}
TEST_END

static void mallctl_long(void) {
    uint64_t nmalloc;
...
static void mallctlbymib_long(void) {
...
TEST_BEGIN(test_mallctl_vs_mallctlbymib_long) {
    ...
    size_t miblen = sizeof(mib_long) / sizeof(mib_long[0]);
    int err = mallctlnametomib("stats.arenas.0.bins.0.nmalloc", mib_long, &miblen);
    assert_d_eq(err, 0, "mallctlnametomib failure");
    compare_funcs(10 * 1000 * 1000, 10 * 1000 * 1000, "mallctl_long", mallctl_long, "mallctlbymib_long", mallctlbymib_long);
}
TEST_END

int main(void) {
    return test_no_reentrancy(test_mallctl_vs_mallctlbymib_short, test_mallctl_vs_mallctlbymib_long);
}
