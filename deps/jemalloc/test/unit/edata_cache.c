...
static void test_edata_cache_init(edata_cache_t *edata_cache) {
    base_t *base = base_new(TSDN_NULL, /* ind */ 1, &ehooks_default_extent_hooks, /* metadata_use_hooks */ true);
    assert_ptr_not_null(base, "");
    bool err = edata_cache_init(edata_cache, base);
    assert_false(err, "");
}

static void test_edata_cache_destroy(edata_cache_t *edata_cache) {
...
static size_t ecf_count(edata_cache_fast_t *ecf) {
    size_t count = 0;
    edata_t *cur;
...
TEST_BEGIN(test_edata_cache_fill) {
    ...
    expect_zu_eq(EDATA_CACHE_FAST_FILL / 2, atomic_load_zu(&ec.count, ATOMIC_RELAXED), "");

    allocs[0] = edata_cache_fast_get(TSDN_NULL, &ecf);
    expect_zu_eq(EDATA_CACHE_FAST_FILL / 2 - 1, ecf_count(&ecf), "Should have grabbed all edatas available but no more.");
    ...
    expect_zu_eq(0, atomic_load_zu(&ec.count, ATOMIC_RELAXED), "Allocated more than necessary");
    ...
    expect_zu_eq(EDATA_CACHE_FAST_FILL, atomic_load_zu(&ec.count, ATOMIC_RELAXED), "");
...
TEST_BEGIN(test_edata_cache_disable) {
    ...
    expect_zu_eq(0, ecf_count(&ecf), "");
    expect_zu_eq(EDATA_CACHE_FAST_FILL, atomic_load_zu(&ec.count, ATOMIC_RELAXED), "Disabling should flush");

    edata_t *edata = edata_cache_fast_get(TSDN_NULL, &ecf);
    expect_zu_eq(0, ecf_count(&ecf), "");
    expect_zu_eq(EDATA_CACHE_FAST_FILL - 1, atomic_load_zu(&ec.count, ATOMIC_RELAXED), "Disabled ecf should forward on get");

    edata_cache_fast_put(TSDN_NULL, &ecf, edata);
    expect_zu_eq(0, ecf_count(&ecf), "");
    expect_zu_eq(EDATA_CACHE_FAST_FILL, atomic_load_zu(&ec.count, ATOMIC_RELAXED), "Disabled ecf should forward on put");
...
int main(void) {
    return test(test_edata_cache, test_edata_cache_fast_simple, test_edata_cache_fill, test_edata_cache_disable);
}
