...
TEST_BEGIN(test_smoothstep_integral) {
    ...
    expect_u64_ge(sum, min, "Integral too small, even accounting for truncation");
...
TEST_BEGIN(test_smoothstep_monotonic) {
    ...
    expect_u64_eq(smoothstep_tab[SMOOTHSTEP_NSTEPS - 1], (KQU(1) << SMOOTHSTEP_BFP), "Last step must equal 1");
}
TEST_END
