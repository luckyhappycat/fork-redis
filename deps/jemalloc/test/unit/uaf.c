...
static void test_write_after_free_pre(void) {
    safety_check_set_abort(&fake_abort);
    fake_abort_called = false;
}

static void test_write_after_free_post(void) {
    assert_d_eq(mallctl("thread.tcache.flush", NULL, NULL, NULL, 0), 0, "Unexpected tcache flush failure");
    expect_true(fake_abort_called, "Use-after-free check didn't fire.");
    safety_check_set_abort(NULL);
}

static bool uaf_detection_enabled(void) {
    ...
    assert_d_eq(mallctl("opt.lg_san_uaf_align", &lg_san_uaf_align, &sz, NULL, 0), 0, "Unexpected mallctl failure");
    ...
    assert_d_eq(mallctl("thread.tcache.enabled", &tcache_enabled, &sz, NULL, 0), 0, "Unexpected mallctl failure");
    if (!tcache_enabled) {
        return false;
    }
...
static size_t read_tcache_stashed_bytes(unsigned arena_ind) {
    if (!config_stats) {
        return 0;
    }
...
static void test_use_after_free(size_t alloc_size, bool write_after_free) {
    ...
    while (iter-- != 0) {
        ...
        if (*mem != magic) {
            junked = true;
            assert_c_eq(*mem, (char)uaf_detect_junk, "Unexpected junk-filling bytes");
            if (write_after_free) {
                *(char *)mem = magic + 1;
            }

            size_t stashed_after = read_tcache_stashed_bytes(arena_ind);
            /*
...
static bool check_allocated_intact(void **allocated, size_t n_alloc) {
...
TEST_BEGIN(test_use_after_free_integration) {
    ...
    *(void **)allocated[0] = allocated[n_alloc - 1];
    expect_true(check_allocated_intact(allocated, n_alloc), "Allocated data corrupted");

    for (unsigned i = 0; i < n_alloc; i++) {
        free(allocated[i]);
    }
    /* Read-after-free */
    expect_false(check_allocated_intact(allocated, n_alloc), "Junk-filling not detected");
...
int main(void) {
    return test(test_read_after_free, test_write_after_free, test_use_after_free_integration);
}
