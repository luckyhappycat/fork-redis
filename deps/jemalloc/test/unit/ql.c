...
static void test_entries_list(list_head_t *head, list_t *entries, unsigned nentries) {
    ...
    expect_c_eq(ql_last(head, link)->id, entries[nentries - 1].id, "Element id mismatch");
    ...
    expect_ptr_null(ql_next(head, &entries[nentries - 1], link), "Unexpected element");
...
static void test_concat_split_entries(list_t *entries, unsigned nentries_a, unsigned nentries_b) {
...
int main(void) {
    return test(
        test_ql_empty, test_ql_tail_insert, test_ql_tail_remove, test_ql_head_insert, test_ql_head_remove, test_ql_insert, test_ql_concat_split,
        test_ql_rotate, test_ql_move
    );
}
