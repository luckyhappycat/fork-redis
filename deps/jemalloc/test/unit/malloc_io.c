...
#undef KSMAX
    ...
    for (i = 0; i < sizeof(tests) / sizeof(struct test_s); i++) {
        ...
        err = get_errno();
        expect_d_eq(err, test->expected_errno, "Expected errno %s for \"%s\", base %d", test->expected_errno_name, test->input, test->base);
        expect_str_eq(remainder, test->expected_remainder, "Unexpected remainder for \"%s\", base %d", test->input, test->base);
        if (err == 0) {
            expect_ju_eq(result, test->expected_x, "Unexpected result for \"%s\", base %d", test->input, test->base);
        }
    }
}
TEST_END
...
#define TEST(expected_str_untruncated, ...)                                                                                                      \
    do {                                                                                                                                         \
        ...
        expect_d_eq(                                                                                                                             \
            strncmp(buf, expected_str_untruncated, len - 1), 0, "Unexpected string inequality (\"%s\" vs \"%s\")", buf, expected_str_untruncated \
        );                                                                                                                                       \
        expect_zu_eq(result, strlen(expected_str_untruncated), "Unexpected result");                                                             \
    } while (0)
