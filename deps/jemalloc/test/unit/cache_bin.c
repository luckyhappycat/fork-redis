...
static void do_fill_test(
    cache_bin_t *bin, cache_bin_info_t *info, void **ptrs, cache_bin_sz_t ncached_max, cache_bin_sz_t nfill_attempt, cache_bin_sz_t nfill_succeed
) {
    bool success;
    void *ptr;
    ...
    for (cache_bin_sz_t i = 0; i < nfill_succeed; i++) {
        ...
        expect_true(cache_bin_low_water_get(bin, info) == nfill_succeed - i - 1, "");
    }
    expect_true(cache_bin_ncached_get_local(bin, info) == 0, "");
    expect_true(cache_bin_low_water_get(bin, info) == 0, "");
}

static void do_flush_test(cache_bin_t *bin, cache_bin_info_t *info, void **ptrs, cache_bin_sz_t nfill, cache_bin_sz_t nflush) {
    bool success;
    assert_true(cache_bin_ncached_get_local(bin, info) == 0, "");
...
static void do_batch_alloc_test(cache_bin_t *bin, cache_bin_info_t *info, void **ptrs, cache_bin_sz_t nfill, size_t batch) {
    assert_true(cache_bin_ncached_get_local(bin, info) == 0, "");
    ...
    expect_true(cache_bin_low_water_get(bin, info) == nfill - (cache_bin_sz_t)n, "");
    while (cache_bin_ncached_get_local(bin, info) > 0) {
        bool success;
        cache_bin_alloc(bin, &success);
    }
...
static void test_bin_init(cache_bin_t *bin, cache_bin_info_t *info) {
    size_t size;
    size_t alignment;
...
TEST_BEGIN(test_cache_bin) {
    ...
    for (cache_bin_sz_t i = 0; i < ncached_max; i++) {
        ...
        success = cache_bin_dalloc_easy(&bin, &ptrs[i]);
        expect_true(success, "Should be able to dalloc into a non-full cache bin.");
        expect_true(cache_bin_low_water_get(&bin, &info) == 0, "Pushes and pops shouldn't change low water of zero.");
    }
    ...
    for (cache_bin_sz_t i = 0; i < ncached_max; i++) {
        expect_true(cache_bin_low_water_get(&bin, &info) == ncached_max - i, "");
        expect_true(cache_bin_ncached_get_local(&bin, &info) == ncached_max - i, "");
        /*
        ...
        ptr = cache_bin_alloc(&bin, &success);
        expect_true(success, "");
        expect_ptr_eq(ptr, &ptrs[ncached_max - i - 1], "Alloc should pop in stack order");
        expect_true(cache_bin_low_water_get(&bin, &info) == ncached_max - i - 1, "");
        expect_true(cache_bin_ncached_get_local(&bin, &info) == ncached_max - i - 1, "");
    }
    ...
    do_fill_test(&bin, &info, ptrs, ncached_max, ncached_max / 2, ncached_max / 2);
    /* Try to fill some, succeed partially. */
    do_fill_test(&bin, &info, ptrs, ncached_max, ncached_max / 2, ncached_max / 4);
...
static void do_flush_stashed_test(cache_bin_t *bin, cache_bin_info_t *info, void **ptrs, cache_bin_sz_t nfill, cache_bin_sz_t nstash) {
    ...
    expect_true(cache_bin_ncached_get_local(bin, info) == nfill, "Wrong cached count");
    ...
    expect_true(cache_bin_nstashed_get_local(bin, info) == nstash, "Wrong stashed count");
    ...
    for (cache_bin_sz_t i = 0; i < nfill; i++) {
        ...
        expect_true((uintptr_t)ptr < (uintptr_t)&ptrs[nfill], "Should not alloc stashed ptrs");
    }
    expect_true(cache_bin_ncached_get_local(bin, info) == 0, "Wrong cached count");
    expect_true(cache_bin_nstashed_get_local(bin, info) == nstash, "Wrong stashed count");
    ...
    expect_true(cache_bin_ncached_get_local(bin, info) == 0, "Wrong cached count");
    expect_true(cache_bin_nstashed_get_local(bin, info) == 0, "Wrong stashed count");
...
TEST_BEGIN(test_cache_bin_stash) {
    ...
    bool ret;
    for (cache_bin_sz_t i = 0; i < ncached_max; i++) {
        expect_true(cache_bin_ncached_get_local(&bin, &info) == (i / 2 + i % 2), "Wrong ncached value");
        expect_true(cache_bin_nstashed_get_local(&bin, &info) == i / 2, "Wrong nstashed value");
        if (i % 2 == 0) {
            cache_bin_dalloc_easy(&bin, &ptrs[i]);
        } else {
    ...
    for (cache_bin_sz_t i = 0; i < ncached_max; i++) {
        ...
        if (i < ncached_max / 2) {
            ...
            uintptr_t diff = ((uintptr_t)ptr - (uintptr_t)&ptrs[0]) / sizeof(void *);
            expect_true(diff % 2 == 0, "Should be able to alloc");
        } else {
            expect_false(ret, "Should not alloc stashed");
            expect_true(cache_bin_nstashed_get_local(&bin, &info) == ncached_max / 2, "Wrong nstashed value");
        }
    }
