...
static void edata_init_test(edata_t *edata) {
    memset(edata, 0, sizeof(*edata));
    edata_arena_ind_set(edata, ALLOC_ARENA_IND);
    edata_esn_set(edata, ALLOC_ESN);
}

static void test_psset_fake_purge(hpdata_t *ps) {
...
static void test_psset_alloc_new(psset_t *psset, hpdata_t *ps, edata_t *r_edata, size_t size) {
    ...
    edata_init(
        r_edata, edata_arena_ind_get(r_edata), addr, size,
...
static bool test_psset_alloc_reuse(psset_t *psset, edata_t *r_edata, size_t size) {
...
static hpdata_t *test_psset_dalloc(psset_t *psset, edata_t *edata) {
...
static void edata_expect(edata_t *edata, size_t page_offset, size_t page_cnt) {
    /*
     * Note that allocations should get the arena ind of their home
     * arena, *not* the arena ind of the pageslab allocator.
     */
    expect_u_eq(ALLOC_ARENA_IND, edata_arena_ind_get(edata), "Arena ind changed");
    expect_ptr_eq((void *)((uintptr_t)PAGESLAB_ADDR + (page_offset << LG_PAGE)), edata_addr_get(edata), "Didn't allocate in order");
...
TEST_BEGIN(test_multi_pageslab) {
    ...
    hpdata_init(&pageslab[1], (void *)((uintptr_t)PAGESLAB_ADDR + HUGEPAGE), PAGESLAB_AGE + 1);
    ...
    for (size_t i = 0; i < 2; i++) {
        for (size_t j = 1; j < HUGEPAGE_PAGES; j++) {
            ...
            expect_false(err, "Nonempty psset failed page allocation.");
            assert_ptr_eq(&pageslab[i], edata_ps_get(&alloc[i][j]), "Didn't pick pageslabs in first-fit");
        }
    }
    ...
    expect_ptr_eq(&pageslab[1], edata_ps_get(&alloc[0][0]), "Should have picked the fuller pageslab");
...
static void stats_expect_empty(psset_bin_stats_t *stats) {
    assert_zu_eq(0, stats->npageslabs, "Supposedly empty bin had positive npageslabs");
    expect_zu_eq(
        0, stats->nactive,
...
static void stats_expect(psset_t *psset, size_t nactive) {
    if (nactive == HUGEPAGE_PAGES) {
        expect_zu_eq(1, psset->stats.full_slabs[0].npageslabs, "Expected a full slab");
        expect_zu_eq(HUGEPAGE_PAGES, psset->stats.full_slabs[0].nactive, "Should have exactly filled the bin");
    } else {
    ...
    if (ninactive != 0 && ninactive < HUGEPAGE_PAGES) {
        nonempty_pind = sz_psz2ind(sz_psz_quantize_floor(ninactive << LG_PAGE));
    }
    for (pszind_t i = 0; i < PSSET_NPSIZES; i++) {
        if (i == nonempty_pind) {
            assert_zu_eq(1, psset->stats.nonfull_slabs[i][0].npageslabs, "Should have found a slab");
            expect_zu_eq(nactive, psset->stats.nonfull_slabs[i][0].nactive, "Mismatch in active pages");
        } else {
 ...
 */
static void init_test_pageslabs(psset_t *psset, hpdata_t *pageslab, hpdata_t *worse_pageslab, edata_t *alloc, edata_t *worse_alloc) {
    bool err;
    ...
    for (size_t i = 1; i < HUGEPAGE_PAGES; i++) {
        ...
        err = test_psset_alloc_reuse(psset, &alloc[i], PAGE);
        expect_false(err, "Nonempty psset failed page allocation.");
        expect_ptr_eq(pageslab, edata_ps_get(&alloc[i]), "Allocated from the wrong pageslab");
    }

    edata_init_test(&worse_alloc[0]);
    test_psset_alloc_new(psset, worse_pageslab, &worse_alloc[0], PAGE);
    expect_ptr_eq(worse_pageslab, edata_ps_get(&worse_alloc[0]), "Allocated from the wrong pageslab");
    /*
    ...
    for (size_t i = 1; i < HUGEPAGE_PAGES - 1; i++) {
        ...
        err = test_psset_alloc_reuse(psset, &alloc[i], PAGE);
        expect_false(err, "Nonempty psset failed page allocation.");
        expect_ptr_eq(worse_pageslab, edata_ps_get(&alloc[i]), "Allocated from the wrong pageslab");
    }

    /* Deallocate the last page from the older pageslab. */
    hpdata_t *evicted = test_psset_dalloc(psset, &alloc[HUGEPAGE_PAGES - 1]);
...
TEST_BEGIN(test_oldest_fit) {
    ...
    init_test_pageslabs(&psset, &pageslab, &worse_pageslab, alloc, worse_alloc);
    ...
    expect_false(err, "Nonempty psset failed page allocation");
    expect_ptr_eq(&pageslab, edata_ps_get(&test_edata), "Allocated from the wrong pageslab");
}
TEST_END
...
TEST_BEGIN(test_insert_remove) {
    ...
    psset_update_begin(&psset, &pageslab);
    err = test_psset_alloc_reuse(&psset, &worse_alloc[HUGEPAGE_PAGES - 1], PAGE);
    expect_false(err, "Removal should still leave an empty page");
    expect_ptr_eq(&worse_pageslab, edata_ps_get(&worse_alloc[HUGEPAGE_PAGES - 1]), "Allocated out of wrong ps");
    ...
    expect_ptr_eq(&pageslab, edata_ps_get(&alloc[HUGEPAGE_PAGES - 1]), "Removal/reinsertion shouldn't change ordering");
    /*
...
TEST_BEGIN(test_purge_prefers_nonhuge) {
    ...
    for (size_t i = 0; i < NHP; i++) {
        hpdata_init(&hpdata_huge[i], (void *)((10 + i) * HUGEPAGE), 123 + i);
        psset_insert(&psset, &hpdata_huge[i]);

        hpdata_init(&hpdata_nonhuge[i], (void *)((10 + NHP + i) * HUGEPAGE), 456 + i);
    ...
    for (int i = 0; i < 2 * NHP; i++) {
        ...
        if (huge_begin <= (uintptr_t)hpdata && (uintptr_t)hpdata < huge_end) {
    ...
    for (int i = 0; i < NHP; i++) {
        ...
        assert_true(nonhuge_begin <= (uintptr_t)hpdata && (uintptr_t)hpdata < nonhuge_end, "");
        psset_update_begin(&psset, hpdata);
    ...
    for (int i = 0; i < NHP; i++) {
        hpdata = psset_pick_purge(&psset);
        expect_true(huge_begin <= (uintptr_t)hpdata && (uintptr_t)hpdata < huge_end, "");
        psset_update_begin(&psset, hpdata);
...
int main(void) {
    return test_no_reentrancy(
        test_empty, test_fill, test_reuse, test_evict, test_multi_pageslab, test_stats, test_oldest_fit, test_insert_remove,
        test_purge_prefers_nonhuge, test_purge_prefers_empty, test_purge_prefers_empty_huge
    );
}
