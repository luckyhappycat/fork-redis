...
static unsigned do_arena_create(extent_hooks_t *h) {
    ...
    expect_d_eq(
        mallctl("arenas.create", (void *)&new_arena_ind, &ind_sz, (void *)(h != NULL ? &h : NULL), (h != NULL ? sizeof(h) : 0)), 0,
        "Unexpected mallctl() failure"
    );
    return new_arena_ind;
}

static void do_arena_destroy(unsigned ind) {
    size_t mib[3];
    size_t miblen;
...
static void do_refresh(void) {
    ...
    expect_d_eq(mallctl("epoch", NULL, NULL, (void *)&refresh_epoch, sizeof(refresh_epoch)), 0, "Unexpected mallctl() failure");
}

static size_t do_get_size_impl(const char *cmd, unsigned ind) {
    size_t mib[4];
    ...
    mib[2] = ind;
    size_t size;
    expect_d_eq(mallctlbymib(mib, miblen, (void *)&size, &z, NULL, 0), 0, "Unexpected mallctlbymib([\"%s\"], ...) failure", cmd);

    return size;
}

static size_t do_get_active(unsigned ind) {
    return do_get_size_impl("stats.arenas.0.pactive", ind) * PAGE;
}

static size_t do_get_mapped(unsigned ind) {
...
TEST_BEGIN(test_retained) {
    ...
    for (unsigned e = 1; e < NEPOCHS; e++) {
        ...
        size_t usable = 0;
        size_t fragmented = 0;
        for (pszind_t pind = sz_psz2ind(HUGEPAGE); pind < arena->pa_shard.pac.exp_grow.next; pind++) {
