...
TEST_BEGIN(test_nstime_compare) {
    ...
    expect_d_eq(nstime_compare(&nsta, &nstb), 1, "nsta should be greater than nstb");
    expect_d_eq(nstime_compare(&nstb, &nsta), -1, "nstb should be less than nsta");

    nstime_init2(&nstb, 42, 44);
    expect_d_eq(nstime_compare(&nsta, &nstb), -1, "nsta should be less than nstb");
    expect_d_eq(nstime_compare(&nstb, &nsta), 1, "nstb should be greater than nsta");
...
TEST_BEGIN(test_nstime_subtract) {
    ...
    nstime_init_zero(&nstb);
    expect_d_eq(nstime_compare(&nsta, &nstb), 0, "Incorrect subtraction result");
...
TEST_BEGIN(test_nstime_imultiply) {
    ...
    expect_d_eq(nstime_compare(&nsta, &nstb), 0, "Incorrect multiplication result");
...
void test_nstime_since_once(nstime_t *t) {
    nstime_t old_t;
    nstime_copy(&old_t, t);
...
int main(void) {
    return test(
        test_nstime_init, test_nstime_init2, test_nstime_copy, test_nstime_compare, test_nstime_add, test_nstime_iadd, test_nstime_subtract,
        test_nstime_isubtract, test_nstime_imultiply, test_nstime_idivide, test_nstime_divide, test_nstime_ns_since, test_nstime_monotonic
    );
}
