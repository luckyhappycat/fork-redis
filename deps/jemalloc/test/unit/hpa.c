...
static hpa_shard_t *create_test_data(hpa_hooks_t *hooks, hpa_shard_opts_t *opts) {
    bool err;
    base_t *base = base_new(TSDN_NULL, /* ind */ SHARD_IND, &ehooks_default_extent_hooks, /* metadata_use_hooks */ true);
    ...
    err = hpa_shard_init(&test_data->shard, &test_data->central, &test_data->emap, test_data->base, &test_data->shard_edata_cache, SHARD_IND, opts);
    assert_false(err, "");

    return (hpa_shard_t *)test_data;
}

static void destroy_test_data(hpa_shard_t *shard) {
...
TEST_BEGIN(test_alloc_max) {
    ...
    hpa_shard_t *shard = create_test_data(&hpa_hooks_default, &test_hpa_shard_opts_default);
    ...
    edata = pai_alloc(tsdn, &shard->pai, ALLOC_MAX, PAGE, false, false, false, &deferred_work_generated);
    expect_ptr_not_null(edata, "Allocation of small max failed");
    edata = pai_alloc(tsdn, &shard->pai, ALLOC_MAX + PAGE, PAGE, false, false, false, &deferred_work_generated);
...
static int mem_contents_cmp(const mem_contents_t *a, const mem_contents_t *b) {
...
static void node_assert_ordered(mem_contents_t *a, mem_contents_t *b) {
    assert_zu_lt(a->my_addr, a->my_addr + a->size, "Overflow");
    assert_zu_le(a->my_addr + a->size, b->my_addr, "");
}

static void node_check(mem_tree_t *tree, mem_contents_t *contents) {
...
static void node_insert(mem_tree_t *tree, edata_t *edata, size_t npages) {
...
static void node_remove(mem_tree_t *tree, edata_t *edata) {
    mem_contents_t *contents = (mem_contents_t *)edata_base_get(edata);
    node_check(tree, contents);
...
TEST_BEGIN(test_stress) {
    ...
    for (size_t i = 0; i < 100 * 1000; i++) {
        ...
        if (operation == 0) {
            ...
            size_t npages = npages_min + prng_range_zu(&prng_state, npages_max - npages_min);
            edata_t *edata = pai_alloc(tsdn, &shard->pai, npages * PAGE, PAGE, false, false, false, &deferred_work_generated);
    ...
    size_t ntreenodes = 0;
    for (mem_contents_t *contents = mem_tree_first(&tree); contents != NULL; contents = mem_tree_next(&tree, contents)) {
        ntreenodes++;
        node_check(&tree, contents);
    }
...
static void expect_contiguous(edata_t **edatas, size_t nedatas) {
    for (size_t i = 0; i < nedatas; i++) {
        size_t expected = (size_t)edata_base_get(edatas[0]) + i * PAGE;
        expect_zu_eq(expected, (size_t)edata_base_get(edatas[i]), "Mismatch at index %zu", i);
    }
}
...
TEST_BEGIN(test_alloc_dalloc_batch) {
    ...
    size_t nsuccess = pai_alloc_batch(tsdn, &shard->pai, PAGE, NALLOCS / 2, &allocs_list, &deferred_work_generated);
...
static void *defer_test_map(size_t size) {
    void *result = (void *)defer_bump_ptr;
    defer_bump_ptr += size;
    return result;
}

static void defer_test_unmap(void *ptr, size_t size) {
    (void)ptr;
    (void)size;
}

static bool defer_purge_called = false;
static void defer_test_purge(void *ptr, size_t size) {
    (void)ptr;
    (void)size;
...
static void defer_test_hugify(void *ptr, size_t size) {
    defer_hugify_called = true;
}

static bool defer_dehugify_called = false;
static void defer_test_dehugify(void *ptr, size_t size) {
    defer_dehugify_called = true;
}

static nstime_t defer_curtime;
static void defer_test_curtime(nstime_t *r_time, bool first_reading) {
    *r_time = defer_curtime;
}

static uint64_t defer_test_ms_since(nstime_t *past_time) {
...
TEST_BEGIN(test_defer_time) {
    ...
    for (int i = 0; i < (int)HUGEPAGE_PAGES; i++) {
        edatas[i] = pai_alloc(tsdn, &shard->pai, PAGE, PAGE, false, false, false, &deferred_work_generated);
...
int main(void) {
    ...
    return test_no_reentrancy(test_alloc_max, test_stress, test_alloc_dalloc_batch, test_defer_time);
}
