...
#define NTICKS 3
    ...
    for (i = 0; i < NREPS; i++) {
        for (j = 0; j < NTICKS; j++) {
            expect_u_eq(ticker_read(&ticker), NTICKS - j, "Unexpected ticker value (i=%d, j=%d)", i, j);
            expect_false(ticker_tick(&ticker), "Unexpected ticker fire (i=%d, j=%d)", i, j);
        }
...
int main(void) {
    return test(test_ticker_tick, test_ticker_ticks, test_ticker_copy, test_ticker_geom);
}
