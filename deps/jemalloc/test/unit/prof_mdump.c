...
static int prof_dump_open_file_intercept(const char *filename, int mode) {
     ...
     */
    expect_ptr_eq(filename, test_filename, "Dump file name should be \"%s\"", test_filename);
...
TEST_BEGIN(test_mdump_normal) {
    ...
    expect_d_eq(mallctl("prof.dump", NULL, NULL, (void *)&test_filename, sizeof(test_filename)), 0, "Unexpected mallctl failure while dumping");
...
static int prof_dump_open_file_error(const char *filename, int mode) {
    return -1;
}
...
static ssize_t prof_dump_write_file_error(int fd, const void *s, size_t len) {
    --prof_dump_write_file_count;

    expect_d_ge(prof_dump_write_file_count, 0, "Write is called after error occurs");
...
static void expect_write_failure(int count) {
    prof_dump_write_file_count = count;
    expect_d_eq(mallctl("prof.dump", NULL, NULL, (void *)&test_filename, sizeof(test_filename)), EFAULT, "Dump should err");
    expect_d_eq(prof_dump_write_file_count, 0, "Dumping stopped after a wrong number of writes");
}
...
static int prof_dump_open_maps_error() {
    return -1;
}

static bool started_piping_maps_file;

static ssize_t prof_dump_write_maps_file_error(int fd, const void *s, size_t len) {
...
static void expect_maps_write_failure(int count) {
...
TEST_BEGIN(test_mdump_maps_error) {
    ...
    expect_d_eq(
        mallctl("prof.dump", NULL, NULL, (void *)&test_filename, sizeof(test_filename)), 0,
...
int main(void) {
    return test(test_mdump_normal, test_mdump_output_error, test_mdump_maps_error);
}
