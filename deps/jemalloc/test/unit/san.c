...
static void verify_extent_guarded(tsdn_t *tsdn, void *ptr) {
    expect_true(extent_is_guarded(tsdn, ptr), "All extents should be guarded.");
}
...
TEST_BEGIN(test_guarded_small) {
    ...
    while (n_alloc < MAX_SMALL_ALLOCATIONS) {
        ...
        if ((uintptr_t)ptr % PAGE == 0) {
            assert_u_lt(pages_found, npages, "Unexpectedly large number of page aligned allocs");
    ...
    for (unsigned i = 0; i < npages - 1; i++) {
        for (unsigned j = i + 1; j < npages; j++) {
            uintptr_t ptr_diff = pages[i] > pages[j] ? pages[i] - pages[j] : pages[j] - pages[i];
            expect_zu_ge(
                (size_t)ptr_diff, slab_sz + PAGE,
...
TEST_BEGIN(test_guarded_large) {
    ...
    for (unsigned i = 0; i < nlarge; i++) {
        for (unsigned j = i + 1; j < nlarge; j++) {
            uintptr_t ptr_diff = large[i] > large[j] ? large[i] - large[j] : large[j] - large[i];
            expect_zu_ge(
                (size_t)ptr_diff, large_sz + 2 * PAGE,
...
static void verify_pdirty(unsigned arena_ind, uint64_t expected) {
    uint64_t pdirty = get_arena_pdirty(arena_ind);
    expect_u64_eq(pdirty, expected / PAGE, "Unexpected dirty page amount.");
}

static void verify_pmuzzy(unsigned arena_ind, uint64_t expected) {
...
TEST_BEGIN(test_guarded_decay) {
    ...
    if (config_stats) {
        expect_u64_eq(get_arena_npurge(arena_ind), 1, "Expected purging to occur");
        expect_u64_eq(get_arena_dirty_npurge(arena_ind), 1, "Expected purging to occur");
        expect_u64_eq(get_arena_dirty_purged(arena_ind), (sz1 + sz2 + 2 * add_guard_size) / PAGE, "Expected purging to occur");
        expect_u64_eq(get_arena_muzzy_npurge(arena_ind), 0, "Expected purging to occur");
    }
