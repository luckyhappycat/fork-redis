...
static void arena_mallctl(const char *mallctl_str, unsigned arena, void *oldp, size_t *oldlen, void *newp, size_t newlen) {
    int err;
    char buf[100];
...
TEST_BEGIN(test_oversize_threshold_get_set) {
    ...
    arena_mallctl("arena.%u.oversize_threshold", arena, NULL, NULL, &new_threshold, threshold_sz);

    /* Read and write */
    new_threshold = 2 * 1024 * 1024;
    arena_mallctl("arena.%u.oversize_threshold", arena, &old_threshold, &threshold_sz, &new_threshold, threshold_sz);
    expect_zu_eq(1024 * 1024, old_threshold, "Should have read old value");

    /* Just a read */
    arena_mallctl("arena.%u.oversize_threshold", arena, &old_threshold, &threshold_sz, NULL, 0);
    expect_zu_eq(2 * 1024 * 1024, old_threshold, "Should have read old value");
}
TEST_END

static size_t max_purged = 0;
static bool purge_forced_record_max(extent_hooks_t *hooks, void *addr, size_t sz, size_t offset, size_t length, unsigned arena_ind) {
...
static bool dalloc_record_max(extent_hooks_t *extent_hooks, void *addr, size_t sz, bool comitted, unsigned arena_ind) {
    if (sz > max_purged) {
        max_purged = sz;
    }
    return false;
...
TEST_BEGIN(test_oversize_threshold) {
    ...
    err = mallctl("arenas.create", (void *)&arena, &arena_sz, NULL, 0);
    expect_d_eq(0, err, "Arena creation failed");
    arena_mallctl("arena.%u.extent_hooks", arena, NULL, NULL, &extent_hooks, sizeof(extent_hooks));
    ...
    arena_mallctl("arena.%u.dirty_decay_ms", arena, NULL, NULL, &decay_ms, decay_ms_sz);
    arena_mallctl("arena.%u.muzzy_decay_ms", arena, NULL, NULL, &decay_ms, decay_ms_sz);
    ...
    arena_mallctl("arena.%u.oversize_threshold", arena, NULL, NULL, &threshold, threshold_sz);
...
int main(void) {
    return test_no_reentrancy(test_oversize_threshold_get_set, test_oversize_threshold);
}
