...
static void test_junk(void *ptr, size_t usize) {
    last_junked_ptr = ptr;
    last_junked_usize = usize;
}

static void do_allocs(size_t size, bool zero, size_t lg_align) {
#define JUNK_ALLOC(...)                                                 \
    do {                                                                \
    ...
    JUNK_ALLOC(mallocx(size, zero_flag | MALLOCX_LG_ALIGN(lg_align) | MALLOCX_TCACHE_NONE));
    ...
    for (size_t i = 0; i < arraylen(zerovals); i++) {
        for (size_t j = 0; j < arraylen(sizevals); j++) {
            for (size_t k = 0; k < arraylen(lg_alignvals); k++) {
                ...
                JUNK_FREE(sdallocx(ptr, usize, MALLOCX_TCACHE_NONE | MALLOCX_LG_ALIGN(lg_align)));
