...
static void *alloc_func(size_t sz) {
    void *ret;
...
static void dalloc_func(void *ptr, size_t sz) {
...
static size_t tcache_bytes_read(void) {
    uint64_t epoch;
...
static void tcache_bytes_check_update(size_t *prev, ssize_t diff) {
...
static void test_tcache_bytes_alloc(size_t alloc_size) {
    expect_d_eq(mallctl("thread.tcache.flush", NULL, NULL, NULL, 0), 0, "Unexpected tcache flush failure");
...
static void test_tcache_max_impl(void) {
    size_t sz;
    ...
    size_t large;
    sz = sizeof(large);
    assert_d_eq(mallctl("arenas.lextent.0.size", (void *)&large, &sz, NULL, 0), 0, "Unexpected mallctl() failure");
...
TEST_BEGIN(test_tcache_max) {
    ...
    for (alloc_option = alloc_option_start; alloc_option < alloc_option_end; alloc_option++) {
        for (dalloc_option = dalloc_option_start; dalloc_option < dalloc_option_end; dalloc_option++) {
