...
TEST_BEGIN(test_small_extent_size) {
    ...
    sz = sizeof(unsigned);
    expect_d_eq(mallctl("arenas.nbins", (void *)&nbins, &sz, NULL, 0), 0, "Unexpected mallctl failure");

    expect_d_eq(mallctlnametomib("arenas.bin.0.slab_size", mib, &miblen), 0, "Unexpected mallctlnametomib failure");
    for (i = 0; i < nbins; i++) {
        mib[2] = i;
        sz = sizeof(size_t);
        expect_d_eq(mallctlbymib(mib, miblen, (void *)&extent_size, &sz, NULL, 0), 0, "Unexpected mallctlbymib failure");
        expect_zu_eq(
            extent_size, sz_psz_quantize_floor(extent_size),
            "Small extent quantization should be a no-op "
            "(extent_size=%zu)",
            extent_size
        );
        expect_zu_eq(
            extent_size, sz_psz_quantize_ceil(extent_size),
            "Small extent quantization should be a no-op "
            "(extent_size=%zu)",
            extent_size
        );
...
TEST_BEGIN(test_large_extent_size) {
    ...
    sz = sizeof(bool);
    expect_d_eq(mallctl("opt.cache_oblivious", (void *)&cache_oblivious, &sz, NULL, 0), 0, "Unexpected mallctl failure");

    sz = sizeof(unsigned);
    expect_d_eq(mallctl("arenas.nlextents", (void *)&nlextents, &sz, NULL, 0), 0, "Unexpected mallctl failure");

    expect_d_eq(mallctlnametomib("arenas.lextent.0.size", mib, &miblen), 0, "Unexpected mallctlnametomib failure");
    for (i = 0; i < nlextents; i++) {
        size_t lextent_size, extent_size, floor, ceil;

        mib[2] = i;
        sz = sizeof(size_t);
        expect_d_eq(mallctlbymib(mib, miblen, (void *)&lextent_size, &sz, NULL, 0), 0, "Unexpected mallctlbymib failure");
        ...
        if (i > 0) {
            expect_zu_eq(extent_size_prev, sz_psz_quantize_floor(extent_size - PAGE), "Floor should be a precise size");
...
#define SZ_MAX ZU(4 * 1024 * 1024)
    ...
    for (i = 1; i <= SZ_MAX >> LG_PAGE; i++) {
        ...
        expect_zu_le(floor, extent_size, "Floor should be <= (floor=%zu, extent_size=%zu, ceil=%zu)", floor, extent_size, ceil);
        expect_zu_ge(
            ceil, extent_size,
