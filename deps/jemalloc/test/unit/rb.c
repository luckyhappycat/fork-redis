...
static uint64_t node_subtree_specialness(node_t *n, const node_t *lchild, const node_t *rchild) {
    uint64_t subtree_specialness = n->specialness;
    if (lchild != NULL && lchild->summary_max_specialness > subtree_specialness) {
        subtree_specialness = lchild->summary_max_specialness;
    }
    if (rchild != NULL && rchild->summary_max_specialness > subtree_specialness) {
        subtree_specialness = rchild->summary_max_specialness;
    }
    return subtree_specialness;
}

static bool node_summarize(node_t *a, const node_t *lchild, const node_t *rchild) {
    uint64_t new_summary_max_specialness = node_subtree_specialness(a, lchild, rchild);
    bool changed = (a->summary_lchild != lchild) || (a->summary_rchild != rchild) || (new_summary_max_specialness != a->summary_max_specialness);
...
rb_summarized_gen(static, tree_, tree_t, node_t, link, node_cmp, node_summarize);

static bool specialness_filter_node(void *ctx, node_t *node) {
    uint64_t specialness = *(uint64_t *)ctx;
    return node->specialness >= specialness;
}

static bool specialness_filter_subtree(void *ctx, node_t *node) {
    uint64_t specialness = *(uint64_t *)ctx;
...
TEST_BEGIN(test_rb_empty) {
    ...
    unsigned nodes = 0;
    tree_iter_filtered(&tree, NULL, &tree_iterate_cb, &nodes, &specialness_filter_node, &specialness_filter_subtree, NULL);
    expect_u_eq(0, nodes, "");

    nodes = 0;
    tree_reverse_iter_filtered(&tree, NULL, &tree_iterate_cb, &nodes, &specialness_filter_node, &specialness_filter_subtree, NULL);
    expect_u_eq(0, nodes, "");

    expect_ptr_null(tree_first_filtered(&tree, &specialness_filter_node, &specialness_filter_subtree, NULL), "");
    expect_ptr_null(tree_last_filtered(&tree, &specialness_filter_node, &specialness_filter_subtree, NULL), "");

    key.key = 0;
    key.magic = NODE_MAGIC;
    expect_ptr_null(tree_search_filtered(&tree, &key, &specialness_filter_node, &specialness_filter_subtree, NULL), "");
    expect_ptr_null(tree_nsearch_filtered(&tree, &key, &specialness_filter_node, &specialness_filter_subtree, NULL), "");
    expect_ptr_null(tree_psearch_filtered(&tree, &key, &specialness_filter_node, &specialness_filter_subtree, NULL), "");
}
TEST_END
...
static unsigned tree_recurse(node_t *node, unsigned black_height, unsigned black_depth) {
    ...
    expect_ptr_eq(left_node, node->summary_lchild, "summary missed a tree update");
    expect_ptr_eq(right_node, node->summary_rchild, "summary missed a tree update");

    uint64_t expected_subtree_specialness = node_subtree_specialness(node, left_node, right_node);
    expect_u64_eq(expected_subtree_specialness, node->summary_max_specialness, "Incorrect summary");
    ...
    if (rbtn_red_get(node_t, link, node)) {
        if (left_node != NULL) {
            expect_false(rbtn_red_get(node_t, link, left_node), "Node should be black");
        }
        if (right_node != NULL) {
            expect_false(rbtn_red_get(node_t, link, right_node), "Node should be black");
        }
    }
...
static void node_remove(tree_t *tree, node_t *node, unsigned nnodes) {
    ...
    expect_u_eq(tree_iterate(tree), nnodes - 1, "Unexpected node iteration count");
    expect_u_eq(tree_iterate_reverse(tree), nnodes - 1, "Unexpected node iteration count");
}
    ...
    for (i = 0; i < NBAGS; i++) {
        ...
        for (j = 1; j <= NNODES; j++) {
            ...
            for (k = 0; k < j; k++) {
                ...
                expect_u_eq(tree_iterate(&tree), k + 1, "Unexpected node iteration count");
                expect_u_eq(tree_iterate_reverse(&tree), k + 1, "Unexpected node iteration count");

                expect_false(tree_empty(&tree), "Tree should not be empty");
                expect_ptr_not_null(tree_first(&tree), "Tree should not be empty");
                expect_ptr_not_null(tree_last(&tree), "Tree should not be empty");
...
static void expect_simple_consistency(tree_t *tree, uint64_t specialness, bool expected_empty, node_t *expected_first, node_t *expected_last) {
    bool empty;
    node_t *first;
    node_t *last;

    empty = tree_empty_filtered(tree, &specialness_filter_node, &specialness_filter_subtree, &specialness);
    expect_b_eq(expected_empty, empty, "");

    first = tree_first_filtered(tree, &specialness_filter_node, &specialness_filter_subtree, (void *)&specialness);
    expect_ptr_eq(expected_first, first, "");

    last = tree_last_filtered(tree, &specialness_filter_node, &specialness_filter_subtree, (void *)&specialness);
...
TEST_BEGIN(test_rb_filter_simple) {
    ...
    expect_simple_consistency(
        &tree, /* specialness */ 1, /* empty */ false,
    ...
    expect_simple_consistency(
        &tree, /* specialness */ 2, /* empty */ false,
        /* first */ &nodes[4], /* last */ &nodes[8]
    );

    /* And a multiple of 8 */
    expect_simple_consistency(
        &tree, /* specialness */ 3, /* empty */ false,
        /* first */ &nodes[8], /* last */ &nodes[8]
    );

    /* But not a multiple of 16 */
    expect_simple_consistency(
        &tree, /* specialness */ 4, /* empty */ true,
        /* first */ NULL, /* last */ NULL
    );
}
TEST_END
...
static node_t *tree_iterate_filtered_cb(tree_t *tree, node_t *node, void *arg) {
    iter_ctx_t *ctx = (iter_ctx_t *)arg;
    ctx->ncalls++;
    expect_u64_ge(node->specialness, 1, "Should only invoke cb on nodes that pass the filter");
    if (ctx->last_node != NULL) {
        if (ctx->forward) {
            expect_d_lt(node_cmp(ctx->last_node, node), 0, "Incorrect iteration order");
        } else {
            expect_d_gt(node_cmp(ctx->last_node, node), 0, "Incorrect iteration order");
        }
    }
    ctx->last_node = node;
...
static int qsort_node_cmp(const void *ap, const void *bp) {
    node_t *a = *(node_t **)ap;
    node_t *b = *(node_t **)bp;
    return node_cmp(a, b);
}
...
static void check_consistency(tree_t *tree, node_t nodes[UPDATE_TEST_MAX], int nnodes) {
    ...
    first = tree_first_filtered(tree, &specialness_filter_node, &specialness_filter_subtree, &specialness);
    expect_ptr_eq(real_first, first, "");

    last = tree_last_filtered(tree, &specialness_filter_node, &specialness_filter_subtree, &specialness);
    ...
    for (int i = 0; i < nnodes; i++) {
        ...
        for (int j = 0; j < nnodes; j++) {
            ...
            if (node_cmp(&nodes[j], &nodes[i]) < 0 && (real_prev_filtered == NULL || node_cmp(&nodes[j], real_prev_filtered) > 0)) {
                real_prev_filtered = &nodes[j];
            }
            if (node_cmp(&nodes[j], &nodes[i]) > 0 && (real_next_filtered == NULL || node_cmp(&nodes[j], real_next_filtered) < 0)) {
                real_next_filtered = &nodes[j];
            }
        }
        next_filtered = tree_next_filtered(tree, &nodes[i], &specialness_filter_node, &specialness_filter_subtree, &specialness);
        expect_ptr_eq(real_next_filtered, next_filtered, "");

        prev_filtered = tree_prev_filtered(tree, &nodes[i], &specialness_filter_node, &specialness_filter_subtree, &specialness);
        ...
        search_filtered = tree_search_filtered(tree, &before, &specialness_filter_node, &specialness_filter_subtree, &specialness);
        expect_ptr_eq(real_search_filtered, search_filtered, "");

        real_nsearch_filtered = (nodes[i].specialness >= specialness ? &nodes[i] : real_next_filtered);
        nsearch_filtered = tree_nsearch_filtered(tree, &before, &specialness_filter_node, &specialness_filter_subtree, &specialness);
        expect_ptr_eq(real_nsearch_filtered, nsearch_filtered, "");

        real_psearch_filtered = real_prev_filtered;
        psearch_filtered = tree_psearch_filtered(tree, &before, &specialness_filter_node, &specialness_filter_subtree, &specialness);
        expect_ptr_eq(real_psearch_filtered, psearch_filtered, "");

        /* search, nsearch, psearch from nodes[i] */
        real_search_filtered = (nodes[i].specialness >= specialness ? &nodes[i] : NULL);
        search_filtered = tree_search_filtered(tree, &nodes[i], &specialness_filter_node, &specialness_filter_subtree, &specialness);
        expect_ptr_eq(real_search_filtered, search_filtered, "");

        real_nsearch_filtered = (nodes[i].specialness >= specialness ? &nodes[i] : real_next_filtered);
        nsearch_filtered = tree_nsearch_filtered(tree, &nodes[i], &specialness_filter_node, &specialness_filter_subtree, &specialness);
        expect_ptr_eq(real_nsearch_filtered, nsearch_filtered, "");

        real_psearch_filtered = (nodes[i].specialness >= specialness ? &nodes[i] : real_prev_filtered);
        psearch_filtered = tree_psearch_filtered(tree, &nodes[i], &specialness_filter_node, &specialness_filter_subtree, &specialness);
        expect_ptr_eq(real_psearch_filtered, psearch_filtered, "");
        ...
        real_search_filtered = (nodes[i].specialness >= specialness ? &nodes[i] : NULL);
        search_filtered = tree_search_filtered(tree, &equiv, &specialness_filter_node, &specialness_filter_subtree, &specialness);
        expect_ptr_eq(real_search_filtered, search_filtered, "");

        real_nsearch_filtered = (nodes[i].specialness >= specialness ? &nodes[i] : real_next_filtered);
        nsearch_filtered = tree_nsearch_filtered(tree, &equiv, &specialness_filter_node, &specialness_filter_subtree, &specialness);
        expect_ptr_eq(real_nsearch_filtered, nsearch_filtered, "");

        real_psearch_filtered = (nodes[i].specialness >= specialness ? &nodes[i] : real_prev_filtered);
        psearch_filtered = tree_psearch_filtered(tree, &equiv, &specialness_filter_node, &specialness_filter_subtree, &specialness);
        expect_ptr_eq(real_psearch_filtered, psearch_filtered, "");
        ...
        real_search_filtered = NULL;
        search_filtered = tree_search_filtered(tree, &after, &specialness_filter_node, &specialness_filter_subtree, &specialness);
        expect_ptr_eq(real_search_filtered, search_filtered, "");

        real_nsearch_filtered = real_next_filtered;
        nsearch_filtered = tree_nsearch_filtered(tree, &after, &specialness_filter_node, &specialness_filter_subtree, &specialness);
        expect_ptr_eq(real_nsearch_filtered, nsearch_filtered, "");

        real_psearch_filtered = (nodes[i].specialness >= specialness ? &nodes[i] : real_prev_filtered);
        psearch_filtered = tree_psearch_filtered(tree, &after, &specialness_filter_node, &specialness_filter_subtree, &specialness);
        expect_ptr_eq(real_psearch_filtered, psearch_filtered, "");
    }
    ...
    iter_result =
        tree_iter_filtered(tree, NULL, &tree_iterate_filtered_cb, &ctx, &specialness_filter_node, &specialness_filter_subtree, &specialness);
    ...
    for (int i = 0; i < nnodes; i++) {
        ...
        ctx.last_node = NULL;
        iter_result =
            tree_iter_filtered(tree, &nodes[i], &tree_iterate_filtered_cb, &ctx, &specialness_filter_node, &specialness_filter_subtree, &specialness);
        expect_ptr_null(iter_result, "");
    ...
    for (int i = 0; i < nnodes; i++) {
        for (int j = 0; j < nspecial - nodes[i].filtered_rank; j++) {
            ...
            ctx.last_node = NULL;
            ctx.ncalls_max = j + 1;
            iter_result = tree_iter_filtered(
                tree, &nodes[i], &tree_iterate_filtered_cb, &ctx, &specialness_filter_node, &specialness_filter_subtree, &specialness
            );
    ...
    iter_result =
        tree_reverse_iter_filtered(tree, NULL, &tree_iterate_filtered_cb, &ctx, &specialness_filter_node, &specialness_filter_subtree, &specialness);
    expect_ptr_null(iter_result, "");
    expect_d_eq(nspecial, ctx.ncalls, "");
    ...
    for (int i = 0; i < nnodes; i++) {
        ctx.ncalls = 0;
        ctx.last_node = NULL;
        iter_result = tree_reverse_iter_filtered(
            tree, &nodes[i], &tree_iterate_filtered_cb, &ctx, &specialness_filter_node, &specialness_filter_subtree, &specialness
        );
        expect_ptr_null(iter_result, "");
    ...
    for (int i = 0; i < nspecial; i++) {
        ...
        ctx.ncalls_max = i + 1;
        iter_result = tree_reverse_iter_filtered(
            tree, NULL, &tree_iterate_filtered_cb, &ctx, &specialness_filter_node, &specialness_filter_subtree, &specialness
        );
    ...
    for (int i = 0; i < nnodes; i++) {
        ...
        for (int j = 0; j < nodes[i].filtered_rank + surplus_rank; j++) {
            ...
            expect_d_eq(j + 1, ctx.ncalls, "");
            expect_ptr_eq(sorted_filtered_nodes[nodes[i].filtered_rank - j - 1 + surplus_rank], iter_result, "");
        }
    }
}

static void do_update_search_test(int nnodes, int ntrees, int nremovals, int nupdates) {
...
rb_gen(static UNUSED, unsummarized_tree_, unsummarized_tree_t, node_t, link, node_cmp);

static node_t *unsummarized_tree_iterate_cb(unsummarized_tree_t *tree, node_t *node, void *data) {
    unsigned *i = (unsigned *)data;
    (*i)++;
    return NULL;
}
...
int main(void) {
    return test_no_reentrancy(test_rb_empty, test_rb_random, test_rb_filter_simple, test_rb_update_search, test_rb_unsummarized);
}
