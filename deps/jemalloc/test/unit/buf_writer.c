...
static void test_write_cb(void *cbopaque, const char *s) {
    ...
    assert_zu_le(prev_test_write_len, test_write_len, "Test write overflowed");
}
...
TEST_BEGIN(test_buf_write_static) {
    ...
    assert_false(
        buf_writer_init(tsdn, &buf_writer, test_write_cb, &arg, test_buf, TEST_BUF_SIZE),
...
TEST_BEGIN(test_buf_write_dynamic) {
    buf_writer_t buf_writer;
    tsdn_t *tsdn = tsdn_fetch();
    assert_false(buf_writer_init(tsdn, &buf_writer, test_write_cb, &arg, NULL, TEST_BUF_SIZE), "buf_writer_init() should not OOM");
    test_buf_writer_body(tsdn, &buf_writer);
}
TEST_END
...
TEST_BEGIN(test_buf_write_oom) {
    buf_writer_t buf_writer;
    tsdn_t *tsdn = tsdn_fetch();
    assert_true(buf_writer_init(tsdn, &buf_writer, test_write_cb, &arg, NULL, SC_LARGE_MAXCLASS + 1), "buf_writer_init() should OOM");
    assert(buf_writer.buf == NULL);
    ...
    for (unit = UNIT_MAX; unit >= 0; unit -= UNIT_MAX / 4) {
        ...
        for (n_unit = 1; n_unit <= 3; ++n_unit) {
            ...
            for (i = 1; i <= n_unit; ++i) {
                arg = prng_lg_range_u64(&arg, 64);
                buf_writer_cb(&buf_writer, s);
                assert_u64_eq(arg_store, arg, "Call back argument didn't get through");
                assert_zu_eq(
                    test_write_len, i * unit,
                    "Incorrect length after writing %zu strings"
                    " of length %zu",
                    i, unit
                );
...
ssize_t test_read_cb(void *cbopaque, void *buf, size_t limit) {
    ...
    if (test_read_count == 0) {
        ...
        assert_zu_le(prev_test_read_len, test_read_len, "Test read overflowed");
        return read_len;
    }
}

static void test_buf_writer_pipe_body(tsdn_t *tsdn, buf_writer_t *buf_writer) {
    arg = 4; /* Starting value of random argument. */
    for (int count = 5; count > 0; --count) {
        ...
        expect_zu_eq(test_write_len, test_read_len, "Write length should be equal to read length");
    }
    buf_writer_terminate(tsdn, buf_writer);
}
...
int main(void) {
    return test(test_buf_write_static, test_buf_write_dynamic, test_buf_write_oom, test_buf_write_pipe, test_buf_write_pipe_oom);
}
