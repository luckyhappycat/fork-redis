...
static void verify_batch_basic(tsd_t *tsd, void **ptrs, size_t batch, size_t usize, bool zero) {
...
static void verify_batch_locality(tsd_t *tsd, void **ptrs, size_t batch, size_t usize, arena_t *arena, unsigned nregs) {
    ...
    for (size_t i = 0, j = 0; i < batch; ++i, ++j) {
        ...
        assert(i > 0);
        void *q = ptrs[i - 1];
        expect_true((uintptr_t)p > (uintptr_t)q && (size_t)((uintptr_t)p - (uintptr_t)q) == usize, "");
    }
}

static void release_batch(void **ptrs, size_t batch, size_t size) {
    for (size_t i = 0; i < batch; ++i) {
...
static size_t batch_alloc_wrapper(void **ptrs, size_t num, size_t size, int flags) {
    batch_alloc_packet_t batch_alloc_packet = {ptrs, num, size, flags};
    size_t filled;
    size_t len = sizeof(size_t);
    assert_d_eq(mallctl("experimental.batch_alloc", &filled, &len, &batch_alloc_packet, sizeof(batch_alloc_packet)), 0, "");
    return filled;
}

static void test_wrapper(size_t size, size_t alignment, bool zero, unsigned arena_flag) {
    tsd_t *tsd = tsd_fetch();
    assert(tsd != NULL);
    const size_t usize = (alignment != 0 ? sz_sa2u(size, alignment) : sz_s2u(size));
    ...
    for (size_t i = 0; i < 4; ++i) {
        ...
        for (int j = -1; j <= 1; ++j) {
            ...
            size_t filled = batch_alloc_wrapper(global_ptrs, batch, size, flags);
...
TEST_BEGIN(test_batch_alloc_manual_arena) {
    ...
    assert_d_eq(mallctl("arenas.create", &arena_ind, &len_unsigned, NULL, 0), 0, "");
...
int main(void) {
    return test(test_batch_alloc, test_batch_alloc_zero, test_batch_alloc_aligned, test_batch_alloc_manual_arena, test_batch_alloc_large);
}
