...
static void nstime_update_mock(nstime_t *time) {
    nupdates_mock++;
    if (monotonic_mock) {
...
TEST_BEGIN(test_decay_ticks) {
    ...
    sz = sizeof(size_t);
    expect_d_eq(mallctl("arenas.lextent.0.size", (void *)&large0, &sz, NULL, 0), 0, "Unexpected mallctl failure");
    ...
    expect_d_eq(
        mallctl("thread.arena", (void *)&old_arena_ind, &sz_arena_ind, (void *)&arena_ind, sizeof(arena_ind)), 0, "Unexpected mallctl() failure"
    );
    decay_ticker = tsd_arena_decay_tickerp_get(tsd_fetch());
    expect_ptr_not_null(decay_ticker, "Unexpected failure getting decay ticker");
    ...
    /* posix_memalign(). */
    tick0 = ticker_geom_read(decay_ticker);
    expect_d_eq(posix_memalign(&p, sizeof(size_t), large0), 0, "Unexpected posix_memalign() failure");
    tick1 = ticker_geom_read(decay_ticker);
    expect_u32_ne(tick1, tick0, "Expected ticker to tick during posix_memalign()");
    free(p);
    ...
    tick1 = ticker_geom_read(decay_ticker);
    expect_u32_ne(tick1, tick0, "Expected ticker to tick during aligned_alloc()");
    free(p);
     ...
     */
    {
        ...
        for (i = 0; i < sizeof(allocx_sizes) / sizeof(size_t); i++) {
            ...
            tick1 = ticker_geom_read(decay_ticker);
            expect_u32_ne(tick1, tick0, "Expected ticker to tick during mallocx() (sz=%zu)", sz);
            /* rallocx(). */
            tick0 = ticker_geom_read(decay_ticker);
            p = rallocx(p, sz, MALLOCX_TCACHE_NONE);
            expect_ptr_not_null(p, "Unexpected rallocx() failure");
            tick1 = ticker_geom_read(decay_ticker);
            expect_u32_ne(tick1, tick0, "Expected ticker to tick during rallocx() (sz=%zu)", sz);
            /* xallocx(). */
            tick0 = ticker_geom_read(decay_ticker);
            xallocx(p, sz, 0, MALLOCX_TCACHE_NONE);
            tick1 = ticker_geom_read(decay_ticker);
            expect_u32_ne(tick1, tick0, "Expected ticker to tick during xallocx() (sz=%zu)", sz);
            /* dallocx(). */
            tick0 = ticker_geom_read(decay_ticker);
            dallocx(p, MALLOCX_TCACHE_NONE);
            tick1 = ticker_geom_read(decay_ticker);
            expect_u32_ne(tick1, tick0, "Expected ticker to tick during dallocx() (sz=%zu)", sz);
            /* sdallocx(). */
            p = mallocx(sz, MALLOCX_TCACHE_NONE);
            expect_ptr_not_null(p, "Unexpected mallocx() failure");
            tick0 = ticker_geom_read(decay_ticker);
    ...
    expect_d_eq(mallctl("arenas.tcache_max", (void *)&tcache_max, &sz_tcache_max, NULL, 0), 0, "Unexpected mallctl() failure");

    sz = sizeof(unsigned);
    expect_d_eq(mallctl("tcache.create", (void *)&tcache_ind, &sz, NULL, 0), 0, "Unexpected mallctl failure");
    ...
    for (i = 0; i < sizeof(tcache_sizes) / sizeof(size_t); i++) {
        ...
        tick0 = ticker_geom_read(decay_ticker);
        expect_d_eq(mallctl("tcache.flush", NULL, NULL, (void *)&tcache_ind, sizeof(unsigned)), 0, "Unexpected mallctl failure");
        tick1 = ticker_geom_read(decay_ticker);

        /* Will only tick if it's in tcache. */
        expect_u32_ne(tick1, tick0, "Expected ticker to tick during tcache flush (sz=%zu)", sz);
    }
}
TEST_END

static void decay_ticker_helper(
    unsigned arena_ind, int flags, bool dirty, ssize_t dt, uint64_t dirty_npurge0, uint64_t muzzy_npurge0, bool terminate_asap
) {
#define NINTERVALS 101
    ...
    do {
        for (unsigned i = 0; i < ARENA_DECAY_NTICKS_PER_UPDATE / 2; i++) {
    ...
    if (config_stats) {
        expect_u64_gt(dirty_npurge1 + muzzy_npurge1, dirty_npurge0 + muzzy_npurge0, "Expected purging to occur");
    }
#undef NINTERVALS
}
...
#define NPS 2048
    ...
    expect_d_eq(mallctl("arenas.lextent.0.size", (void *)&large, &sz, NULL, 0), 0, "Unexpected mallctl failure");
    ...
    for (unsigned i = 0; i < NPS; i += 2) {
        ...
        do_decay(arena_ind);
        expect_u_gt(nupdates_mock, nupdates0, "Expected nstime_update() to be called");
    }
...
#define NPS (SMOOTHSTEP_NSTEPS + 1)
    ...
    for (i = 0; i < NPS; i++) {
        ...
        expect_d_eq(mallctl("arena.0.decay", NULL, NULL, NULL, 0), 0, "Unexpected arena.0.decay failure");
        expect_u_gt(nupdates_mock, nupdates0, "Expected nstime_update() to be called");
    }
...
TEST_BEGIN(test_decay_never) {
    ...
    for (unsigned i = 0; i < sizeof(sizes) / sizeof(size_t); i++) {
        ...
        expect_zu_gt(pdirty + (size_t)get_arena_dirty_purged(arena_ind), pdirty_prev, "Expected dirty pages to increase.");
