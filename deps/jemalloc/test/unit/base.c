...
TEST_BEGIN(test_base_hooks_default) {
    ...
    if (config_stats) {
        ...
        expect_zu_ge(allocated0, sizeof(base_t), "Base header should count as allocated");
        if (opt_metadata_thp == metadata_thp_always) {
            expect_zu_gt(n_thp, 0, "Base should have 1 THP at least.");
        }
    }

    expect_ptr_not_null(base_alloc(tsdn, base, 42, 1), "Unexpected base_alloc() failure");

    if (config_stats) {
        base_stats_get(tsdn, base, &allocated1, &resident, &mapped, &n_thp);
        expect_zu_ge(allocated1 - allocated0, 42, "At least 42 bytes were allocated by base_alloc()");
    }
...
TEST_BEGIN(test_base_hooks_not_null) {
     ...
     */
    {
        ...
        for (i = 0; i < sizeof(alignments) / sizeof(size_t); i++) {
            ...
            expect_ptr_eq(p, (void *)(ALIGNMENT_CEILING((uintptr_t)p, alignment)), "Expected quantum alignment");
            q = base_alloc(tsdn, base, alignment, alignment);
            expect_ptr_not_null(q, "Unexpected base_alloc() failure");
            expect_ptr_eq((void *)((uintptr_t)p + align_ceil), q, "Minimal allocation should take up %zu bytes", align_ceil);
            r = base_alloc(tsdn, base, 1, alignment);
            expect_ptr_not_null(r, "Unexpected base_alloc() failure");
            expect_ptr_eq((void *)((uintptr_t)q + align_ceil), r, "Minimal allocation should take up %zu bytes", align_ceil);
        }
    }
     ...
     * allocation.
     */
    expect_zu_ge(edata_bsize_get(&base->blocks->edata), QUANTUM, "Remainder insufficient for test");
     ...
     */
    {
        ...
        for (i = 0; i < sizeof(alignments) / sizeof(size_t); i++) {
            ...
            expect_ptr_not_null(p, "Unexpected base_alloc() failure");
            expect_ptr_eq(p, (void *)(ALIGNMENT_CEILING((uintptr_t)p, alignment)), "Expected %zu-byte alignment", alignment);
        }
    }
...
TEST_BEGIN(test_base_ehooks_get_for_metadata_default_hook) {
    ...
    expect_true(ehooks_are_default(ehooks), "Expected default extent hook functions pointer");
    base_delete(tsdn, base);
}
TEST_END
...
TEST_BEGIN(test_base_ehooks_get_for_metadata_custom_hook) {
    ...
    base = base_new(tsdn, 0, &hooks, /* metadata_use_hooks */ true);
    ehooks_t *ehooks = base_ehooks_get_for_metadata(base);
    expect_ptr_eq(&hooks, ehooks_get_extent_hooks_ptr(ehooks), "Expected user-specified extend hook functions pointer");
    base_delete(tsdn, base);
}
TEST_END
...
int main(void) {
    return test(
        test_base_hooks_default, test_base_hooks_null, test_base_hooks_not_null, test_base_ehooks_get_for_metadata_default_hook,
