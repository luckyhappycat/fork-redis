...
TEST_BEGIN(test_prng_lg_range_u32) {
    ...
    expect_u32_eq(ra, rb, "Repeated generation should produce repeated results");

    sb = 42;
    rb = prng_lg_range_u32(&sb, 32);
    expect_u32_eq(ra, rb, "Equivalent generation should produce equivalent results");
    ...
    for (lg_range = 31; lg_range > 0; lg_range--) {
        ...
        expect_u32_eq((rb & (UINT32_C(0xffffffff) << lg_range)), 0, "High order bits should be 0, lg_range=%u", lg_range);
        expect_u32_eq(
            rb, (ra >> (32 - lg_range)),
...
TEST_BEGIN(test_prng_lg_range_u64) {
    ...
    expect_u64_eq(ra, rb, "Repeated generation should produce repeated results");

    sb = 42;
    rb = prng_lg_range_u64(&sb, 64);
    expect_u64_eq(ra, rb, "Equivalent generation should produce equivalent results");
    ...
    for (lg_range = 63; lg_range > 0; lg_range--) {
        ...
        expect_u64_eq((rb & (UINT64_C(0xffffffffffffffff) << lg_range)), 0, "High order bits should be 0, lg_range=%u", lg_range);
        expect_u64_eq(
            rb, (ra >> (64 - lg_range)),
            "Expected high order bits of full-width result, "
            "lg_range=%u",
...
TEST_BEGIN(test_prng_lg_range_zu) {
    ...
    expect_zu_eq(ra, rb, "Equivalent generation should produce equivalent results");
    ...
    for (lg_range = (ZU(1) << (3 + LG_SIZEOF_PTR)) - 1; lg_range > 0; lg_range--) {
        ...
        expect_zu_eq((rb & (SIZE_T_MAX << lg_range)), 0, "High order bits should be 0, lg_range=%u", lg_range);
        expect_zu_eq(
...
int main(void) {
    return test_no_reentrancy(
        test_prng_lg_range_u32, test_prng_lg_range_u64, test_prng_lg_range_zu, test_prng_range_u32, test_prng_range_u64, test_prng_range_zu
    );
}
