...
void *thd_start(void *arg) {
    ...
    expect_ptr_eq(ap0, ap1, "Pointer returned by \"thread.allocatedp\" should not change");
    ...
    expect_ptr_eq(dp0, dp1, "Pointer returned by \"thread.deallocatedp\" should not change");
    ...
    return NULL;
label_ENOENT:
    expect_false(config_stats, "ENOENT should only be returned if stats are disabled");
