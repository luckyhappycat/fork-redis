 ...
 */
static bool malloc_cpu_count_is_deterministic() {
#ifdef _WIN32
    return true;
#else
...
static void init_opt_stats_opts(const char *v, size_t vlen, char *dest) {
...
static bool malloc_conf_multi_sizes_next(
    const char **slab_size_segment_cur, size_t *vlen_left, size_t *slab_start, size_t *slab_end, size_t *new_size
) {
...
static void malloc_slow_flag_init(void) {
     ...
     */
    malloc_slow_flags |= (opt_junk_alloc ? flag_opt_junk_alloc : 0) | (opt_junk_free ? flag_opt_junk_free : 0) | (opt_zero ? flag_opt_zero : 0) |
...
static void malloc_conf_init_helper(
    sc_data_t *sc_data, unsigned bin_shard_sizes[SC_NBINS], bool initial_call, const char *opts_cache[MALLOC_CONF_NSOURCES], char buf[PATH_MAX + 1]
) {
...
#define CONF_VALUE_READ_FAIL() (get_errno() != 0 || (uintptr_t)end - (uintptr_t)v != vlen)
...
#define CONF_HANDLE_T_U(t, o, n, min, max, check_min, check_max, clip) CONF_HANDLE_T(t, uintmax_t, o, n, min, max, check_min, check_max, clip)
#define CONF_HANDLE_T_SIGNED(t, o, n, min, max, check_min, check_max, clip) CONF_HANDLE_T(t, intmax_t, o, n, min, max, check_min, check_max, clip)
...
#define CONF_HANDLE_INT64_T(o, n, min, max, check_min, check_max, clip) CONF_HANDLE_T_SIGNED(int64_t, o, n, min, max, check_min, check_max, clip)
#define CONF_HANDLE_UINT64_T(o, n, min, max, check_min, check_max, clip) CONF_HANDLE_T_U(uint64_t, o, n, min, max, check_min, check_max, clip)
#define CONF_HANDLE_SSIZE_T(o, n, min, max) CONF_HANDLE_T_SIGNED(ssize_t, o, n, min, max, CONF_CHECK_MIN, CONF_CHECK_MAX, false)
#define CONF_HANDLE_CHAR_P(o, n, d)                                     \
    if (CONF_MATCH(n)) {                                                \
        ...
        CONF_CONTINUE;                                                  \
            ...
            if (CONF_MATCH("narenas")) {
                if (CONF_MATCH_VALUE("default")) {
                    ...
                    CONF_HANDLE_UNSIGNED(
                        opt_narenas, "narenas", 1, UINT_MAX, CONF_CHECK_MIN, CONF_DONT_CHECK_MAX,
                        /* clip */ false
                    )
                }
            }
            ...
            CONF_HANDLE_INT64_T(opt_mutex_max_spin, "mutex_max_spin", -1, INT64_MAX, CONF_CHECK_MIN, CONF_DONT_CHECK_MAX, false);
            CONF_HANDLE_SSIZE_T(
                opt_dirty_decay_ms, "dirty_decay_ms", -1, NSTIME_SEC_MAX * KQU(1000) < QU(SSIZE_MAX) ? NSTIME_SEC_MAX * KQU(1000) : SSIZE_MAX
            );
            CONF_HANDLE_SSIZE_T(
                opt_muzzy_decay_ms, "muzzy_decay_ms", -1, NSTIME_SEC_MAX * KQU(1000) < QU(SSIZE_MAX) ? NSTIME_SEC_MAX * KQU(1000) : SSIZE_MAX
            );
            ...
            CONF_HANDLE_INT64_T(opt_stats_interval, "stats_interval", -1, INT64_MAX, CONF_CHECK_MIN, CONF_DONT_CHECK_MAX, false)
            ...
            if (config_enable_cxx) {
                CONF_HANDLE_BOOL(opt_experimental_infallible_new, "experimental_infallible_new")
            }

            CONF_HANDLE_BOOL(opt_tcache, "tcache")
            CONF_HANDLE_SIZE_T(opt_tcache_max, "tcache_max", 0, TCACHE_MAXCLASS_LIMIT, CONF_DONT_CHECK_MIN, CONF_CHECK_MAX, /* clip */ true)
             ...
             * deeply confused.
             */
            CONF_HANDLE_SSIZE_T(opt_lg_tcache_nslots_mul, "lg_tcache_nslots_mul", -16, 16)
            /* Ditto with values past 2048. */
            CONF_HANDLE_UNSIGNED(opt_tcache_nslots_small_min, "tcache_nslots_small_min", 1, 2048, CONF_CHECK_MIN, CONF_CHECK_MAX, /* clip */ true)
            CONF_HANDLE_UNSIGNED(opt_tcache_nslots_small_max, "tcache_nslots_small_max", 1, 2048, CONF_CHECK_MIN, CONF_CHECK_MAX, /* clip */ true)
            CONF_HANDLE_UNSIGNED(opt_tcache_nslots_large, "tcache_nslots_large", 1, 2048, CONF_CHECK_MIN, CONF_CHECK_MAX, /* clip */ true)
            CONF_HANDLE_SIZE_T(
                opt_tcache_gc_incr_bytes, "tcache_gc_incr_bytes", 1024, SIZE_T_MAX, CONF_CHECK_MIN, CONF_DONT_CHECK_MAX,
                /* clip */ true
            )
            CONF_HANDLE_SIZE_T(
                opt_tcache_gc_delay_bytes, "tcache_gc_delay_bytes", 0, SIZE_T_MAX, CONF_DONT_CHECK_MIN, CONF_DONT_CHECK_MAX,
                /* clip */ false
            )
            CONF_HANDLE_UNSIGNED(opt_lg_tcache_flush_small_div, "lg_tcache_flush_small_div", 1, 16, CONF_CHECK_MIN, CONF_CHECK_MAX, /* clip */ true)
            CONF_HANDLE_UNSIGNED(opt_lg_tcache_flush_large_div, "lg_tcache_flush_large_div", 1, 16, CONF_CHECK_MIN, CONF_CHECK_MAX, /* clip */ true)
            ...
            CONF_HANDLE_SIZE_T(
                opt_lg_extent_max_active_fit, "lg_extent_max_active_fit", 0, (sizeof(size_t) << 3), CONF_DONT_CHECK_MIN, CONF_CHECK_MAX, false
            )

            if (strncmp("percpu_arena", k, klen) == 0) {
                bool match = false;
                for (int m = percpu_arena_mode_names_base; m < percpu_arena_mode_names_limit; m++) {
            ...
            CONF_HANDLE_SIZE_T(
                opt_max_background_threads, "max_background_threads", 1, opt_max_background_threads, CONF_CHECK_MIN, CONF_CHECK_MAX, true
            );
            CONF_HANDLE_BOOL(opt_hpa, "hpa")
            CONF_HANDLE_SIZE_T(opt_hpa_opts.slab_max_alloc, "hpa_slab_max_alloc", PAGE, HUGEPAGE, CONF_CHECK_MIN, CONF_CHECK_MAX, true);
             ...
             */
            CONF_HANDLE_SIZE_T(
                opt_hpa_opts.hugification_threshold, "hpa_hugification_threshold", PAGE, HUGEPAGE, CONF_CHECK_MIN, CONF_CHECK_MAX, true
            );
            if (CONF_MATCH("hpa_hugification_threshold_ratio")) {
                fxp_t ratio;
                char *end;
                bool err = fxp_parse(&ratio, v, &end);
                if (err || (size_t)(end - v) != vlen || ratio > FXP_INIT_INT(1)) {
                    CONF_ERROR("Invalid conf value", k, klen, v, vlen);
                } else {
                    opt_hpa_opts.hugification_threshold = fxp_mul_frac(HUGEPAGE, ratio);
                }
                CONF_CONTINUE;
            }

            CONF_HANDLE_UINT64_T(opt_hpa_opts.hugify_delay_ms, "hpa_hugify_delay_ms", 0, 0, CONF_DONT_CHECK_MIN, CONF_DONT_CHECK_MAX, false);

            CONF_HANDLE_UINT64_T(
                opt_hpa_opts.min_purge_interval_ms, "hpa_min_purge_interval_ms", 0, 0, CONF_DONT_CHECK_MIN, CONF_DONT_CHECK_MAX, false
            );
            ...
            CONF_HANDLE_SIZE_T(opt_hpa_sec_opts.nshards, "hpa_sec_nshards", 0, 0, CONF_CHECK_MIN, CONF_DONT_CHECK_MAX, true);
            CONF_HANDLE_SIZE_T(opt_hpa_sec_opts.max_alloc, "hpa_sec_max_alloc", PAGE, 0, CONF_CHECK_MIN, CONF_DONT_CHECK_MAX, true);
            CONF_HANDLE_SIZE_T(opt_hpa_sec_opts.max_bytes, "hpa_sec_max_bytes", PAGE, 0, CONF_CHECK_MIN, CONF_DONT_CHECK_MAX, true);
            CONF_HANDLE_SIZE_T(opt_hpa_sec_opts.bytes_after_flush, "hpa_sec_bytes_after_flush", PAGE, 0, CONF_CHECK_MIN, CONF_DONT_CHECK_MAX, true);
            CONF_HANDLE_SIZE_T(
                opt_hpa_sec_opts.batch_fill_extra, "hpa_sec_batch_fill_extra", 0, HUGEPAGE_PAGES, CONF_CHECK_MIN, CONF_CHECK_MAX, true
            );
            ...
            if (config_prof) {
                ...
                CONF_HANDLE_SSIZE_T(opt_prof_recent_alloc_max, "prof_recent_alloc_max", -1, SSIZE_MAX)
                CONF_HANDLE_BOOL(opt_prof_stats, "prof_stats")
                CONF_HANDLE_BOOL(opt_prof_sys_thread_name, "prof_sys_thread_name")
            ...
            CONF_HANDLE_SIZE_T(opt_san_guard_small, "san_guard_small", 0, SIZE_T_MAX, CONF_DONT_CHECK_MIN, CONF_DONT_CHECK_MAX, false)
            CONF_HANDLE_SIZE_T(opt_san_guard_large, "san_guard_large", 0, SIZE_T_MAX, CONF_DONT_CHECK_MIN, CONF_DONT_CHECK_MAX, false)
...
static bool malloc_conf_init_check_deps(void) {
...
static void malloc_conf_init(sc_data_t *sc_data, unsigned bin_shard_sizes[SC_NBINS]) {
    const char *opts_cache[MALLOC_CONF_NSOURCES] = {NULL, NULL, NULL, NULL, NULL};
    char buf[PATH_MAX + 1];
...
static bool malloc_init_hard_a0_locked() {
    ...
    if (opt_hpa && !hpa_supported()) {
        ...
        if (pa_shard_enable_hpa(TSDN_NULL, &a0->pa_shard, &hpa_shard_opts, &opt_hpa_sec_opts)) {
            return true;
        }
    }
...
#define UNLOCK_RETURN(tsdn, ret, reentrancy)    \
    ...
    if (malloc_init_narenas() || background_thread_boot1(tsd_tsdn(tsd), b0get())) {
 ...
 */
JEMALLOC_ALWAYS_INLINE bool aligned_usize_get(size_t size, size_t alignment, size_t *usize, szind_t *ind, bool bump_empty_aligned_alloc) {
    assert(usize != NULL);
    if (alignment == 0) {
        if (ind != NULL) {
...
JEMALLOC_ALWAYS_INLINE tcache_t *tcache_get_from_ind(tsd_t *tsd, unsigned tcache_ind, bool slow, bool is_alloc) {
    tcache_t *tcache;
    if (tcache_ind == TCACHE_IND_AUTOMATIC) {
         ...
         * reentrant.
         */
        assert(is_alloc || tsd_reentrancy_level_get(tsd) == 0 || tsd_state_nocleanup(tsd));
...
JEMALLOC_ALWAYS_INLINE bool maybe_check_alloc_ctx(tsd_t *tsd, void *ptr, emap_alloc_ctx_t *alloc_ctx) {
    ...
    if (likely(ptr != NULL)) {
        ...
        if (likely(tsd_fast(tsd))) {
            tcache_t *tcache = tcache_get_from_ind(
                tsd, TCACHE_IND_AUTOMATIC, /* slow */ false,
                /* is_alloc */ false
            );
            ifree(tsd, ptr, tcache, /* slow */ false);
        } else {
            tcache_t *tcache = tcache_get_from_ind(
                tsd, TCACHE_IND_AUTOMATIC, /* slow */ true,
                /* is_alloc */ false
            );
...
JEMALLOC_ALWAYS_INLINE bool free_fastpath_nonfast_aligned(void *ptr, bool check_prof) {
    /*
...
bool free_fastpath(void *ptr, size_t size, bool size_hint) {
     ...
     */
    assert(tsd_fast(tsd) || *tsd_thread_deallocated_next_event_fastp_get_unsafe(tsd) == 0);

    emap_alloc_ctx_t alloc_ctx;
    if (!size_hint) {
        bool err = emap_alloc_ctx_try_lookup_fast(tsd, &arena_emap_global, ptr, &alloc_ctx);
         ...
         */
        if (unlikely(
                size > SC_LOOKUP_MAXCLASS || free_fastpath_nonfast_aligned(
                                                 ptr,
                                                 /* check_prof */ true
                                             )
            )) {
...
static void *irallocx_prof_sample(
    tsdn_t *tsdn, void *old_ptr, size_t old_usize, size_t usize, size_t alignment, bool zero, tcache_t *tcache, arena_t *arena, prof_tctx_t *tctx,
...
JEMALLOC_ALWAYS_INLINE void *irallocx_prof(
    tsd_t *tsd, void *old_ptr, size_t old_usize, size_t size, size_t alignment, size_t usize, bool zero, tcache_t *tcache, arena_t *arena,
    ...
    prof_realloc(tsd, p, size, usize, tctx, prof_active, old_ptr, old_usize, &old_prof_info, sample_event);
...
ixallocx_prof(tsd_t *tsd, void *ptr, size_t old_usize, size_t size, size_t extra, size_t alignment, bool zero, emap_alloc_ctx_t *alloc_ctx) {
    /*
    ...
    if (usize == old_usize) {
        ...
        prof_realloc(tsd, ptr, size, usize, tctx, prof_active, ptr, old_usize, &prof_info, sample_event);
    }
...
size_t batch_alloc(void **ptrs, size_t num, size_t size, int flags) {
    LOG("core.batch_alloc.entry", "ptrs: %p, num: %zu, size: %zu, flags: %d", ptrs, num, size, flags);
    ...
    while (filled < num) {
        ...
        bool prof_sample_event =
            config_prof && opt_prof && prof_active_get_unlocked() && te_prof_sample_event_lookahead_surplus(tsd, batch * usize, &surplus);
        ...
        if (likely(ind < SC_NBINS) && batch >= nregs) {
            ...
            size_t n = arena_fill_small_fresh(tsd_tsdn(tsd), arena, ind, ptrs + filled, arena_batch, zero);
            progress += n;
            filled += n;
        }
