...
void *large_palloc(tsdn_t *tsdn, arena_t *arena, size_t usize, size_t alignment, bool zero) {
    ...
    if (unlikely(arena == NULL) || (edata = arena_extent_alloc_large(tsdn, arena, usize, alignment, zero)) == NULL) {
        return NULL;
    }
...
static bool large_ralloc_no_move_shrink(tsdn_t *tsdn, edata_t *edata, size_t usize) {
    ...
    bool err = pa_shrink(tsdn, &arena->pa_shard, edata, old_size, usize + sz_large_pad, sz_size2index(usize), &deferred_work_generated);
    if (err) {
        return true;
    }
...
static bool large_ralloc_no_move_expand(tsdn_t *tsdn, edata_t *edata, size_t usize, bool zero) {
    arena_t *arena = arena_get_from_edata(edata);
    ...
    bool deferred_work_generated = false;
    bool err = pa_expand(tsdn, &arena->pa_shard, edata, old_size, new_size, szind, zero, &deferred_work_generated);
    ...
    if (zero) {
        if (opt_cache_oblivious) {
             ...
             */
            void *zbase = (void *)((uintptr_t)edata_addr_get(edata) + old_usize);
...
bool large_ralloc_no_move(tsdn_t *tsdn, edata_t *edata, size_t usize_min, size_t usize_max, bool zero) {
    ...
    if (usize_max > oldusize) {
        ...
        if (usize_min < usize_max && usize_min > oldusize && large_ralloc_no_move_expand(tsdn, edata, usize_min, zero)) {
            arena_decay_tick(tsdn, arena_get_from_edata(edata));
            return false;
        }
    }
...
void *large_ralloc(
    tsdn_t *tsdn, arena_t *arena, void *ptr, size_t usize, size_t alignment, bool zero, tcache_t *tcache, hook_ralloc_args_t *hook_args
) {
 ...
 */
static void large_dalloc_prep_impl(tsdn_t *tsdn, arena_t *arena, edata_t *edata, bool locked) {
    if (!locked) {
        /* See comments in arena_bin_slabs_full_insert(). */
        if (!arena_is_auto(arena)) {
            malloc_mutex_lock(tsdn, &arena->large_mtx);
...
static void large_dalloc_finish_impl(tsdn_t *tsdn, arena_t *arena, edata_t *edata) {
    bool deferred_work_generated = false;
...
void large_prof_info_get(tsd_t *tsd, edata_t *edata, prof_info_t *prof_info, bool reset_recent) {
...
static void large_prof_tctx_set(edata_t *edata, prof_tctx_t *tctx) {
