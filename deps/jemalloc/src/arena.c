...
static void arena_bin_lower_slab(tsdn_t *tsdn, arena_t *arena, edata_t *slab, bin_t *bin);
static void arena_maybe_do_deferred_work(tsdn_t *tsdn, arena_t *arena, decay_t *decay, size_t npages_new);

/******************************************************************************/

void arena_basic_stats_merge(
    tsdn_t *tsdn, arena_t *arena, unsigned *nthreads, const char **dss, ssize_t *dirty_decay_ms, ssize_t *muzzy_decay_ms, size_t *nactive,
...
void arena_stats_merge(
    tsdn_t *tsdn, arena_t *arena, unsigned *nthreads, const char **dss, ssize_t *dirty_decay_ms, ssize_t *muzzy_decay_ms, size_t *nactive,
    size_t *ndirty, size_t *nmuzzy, arena_stats_t *astats, bin_stats_data_t *bstats, arena_stats_large_t *lstats, pac_estats_t *estats,
    ...
    for (szind_t i = 0; i < SC_NSIZES - SC_NBINS; i++) {
        uint64_t nmalloc = locked_read_u64(tsdn, LOCKEDINT_MTX(arena->stats.mtx), &arena->stats.lstats[i].nmalloc);
        locked_inc_u64_unsynchronized(&lstats[i].nmalloc, nmalloc);
        astats->nmalloc_large += nmalloc;

        uint64_t ndalloc = locked_read_u64(tsdn, LOCKEDINT_MTX(arena->stats.mtx), &arena->stats.lstats[i].ndalloc);
        locked_inc_u64_unsynchronized(&lstats[i].ndalloc, ndalloc);
        astats->ndalloc_large += ndalloc;

        uint64_t nrequests = locked_read_u64(tsdn, LOCKEDINT_MTX(arena->stats.mtx), &arena->stats.lstats[i].nrequests);
        locked_inc_u64_unsynchronized(&lstats[i].nrequests, nmalloc + nrequests);
        ...
        uint64_t nflush = locked_read_u64(tsdn, LOCKEDINT_MTX(arena->stats.mtx), &arena->stats.lstats[i].nflushes);
    ...
    pa_shard_stats_merge(tsdn, &arena->pa_shard, &astats->pa_shard_stats, estats, hpastats, secstats, &astats->resident);
    ...
    ql_foreach(descriptor, &arena->cache_bin_array_descriptor_ql, link) {
        for (szind_t i = 0; i < nhbins; i++) {
            ...
            cache_bin_nitems_get_remote(cache_bin, &tcache_bin_info[i], &ncached, &nstashed);
...
static void arena_background_thread_inactivity_check(tsdn_t *tsdn, arena_t *arena, bool is_background_thread) {
    if (!background_thread_enabled() || is_background_thread) {
        return;
    }
    background_thread_info_t *info = arena_background_thread_info_get(arena);
    if (background_thread_indefinite_sleep(info)) {
        arena_maybe_do_deferred_work(tsdn, arena, &arena->pa_shard.pac.decay_dirty, 0);
    }
}
...
static void *arena_slab_reg_alloc(edata_t *slab, const bin_info_t *bin_info) {
    ...
    ret = (void *)((uintptr_t)edata_addr_get(slab) + (uintptr_t)(bin_info->reg_size * regind));
...
static void arena_slab_reg_alloc_batch(edata_t *slab, const bin_info_t *bin_info, unsigned cnt, void **ptrs) {
    ...
    for (unsigned i = 0; i < cnt; i++) {
        ...
        *(ptrs + i) = (void *)((uintptr_t)edata_addr_get(slab) + (uintptr_t)(bin_info->reg_size * regind));
    }
#else
    unsigned group = 0;
...
static void arena_large_malloc_stats_update(tsdn_t *tsdn, arena_t *arena, size_t usize) {
    ...
    locked_inc_u64(tsdn, LOCKEDINT_MTX(arena->stats.mtx), &arena->stats.lstats[hindex].nmalloc, 1);
}
...
static void arena_large_dalloc_stats_update(tsdn_t *tsdn, arena_t *arena, size_t usize) {
    ...
    locked_inc_u64(tsdn, LOCKEDINT_MTX(arena->stats.mtx), &arena->stats.lstats[hindex].ndalloc, 1);
}
...
edata_t *arena_extent_alloc_large(tsdn_t *tsdn, arena_t *arena, size_t usize, size_t alignment, bool zero) {
    bool deferred_work_generated = false;
    szind_t szind = sz_size2index(usize);
    size_t esize = usize + sz_large_pad;

    bool guarded = san_large_extent_decide_guard(tsdn, arena_get_ehooks(arena), esize, alignment);
    edata_t *edata = pa_alloc(
        tsdn, &arena->pa_shard, esize, alignment,
...
void arena_extent_dalloc_large_prep(tsdn_t *tsdn, arena_t *arena, edata_t *edata) {
    if (config_stats) {
        LOCKEDINT_MTX_LOCK(tsdn, arena->stats.mtx);
...
void arena_extent_ralloc_large_shrink(tsdn_t *tsdn, arena_t *arena, edata_t *edata, size_t oldusize) {
...
void arena_extent_ralloc_large_expand(tsdn_t *tsdn, arena_t *arena, edata_t *edata, size_t oldusize) {
    size_t usize = edata_usize_get(edata);
 ...
 */
static pac_purge_eagerness_t arena_decide_unforced_purge_eagerness(bool is_background_thread) {
...
bool arena_decay_ms_set(tsdn_t *tsdn, arena_t *arena, extent_state_t state, ssize_t decay_ms) {
...
static bool arena_decay_impl(
    tsdn_t *tsdn, arena_t *arena, decay_t *decay, pac_decay_stats_t *decay_stats, ecache_t *ecache, bool is_background_thread, bool all
) {
    if (all) {
        malloc_mutex_lock(tsdn, &decay->mtx);
        pac_decay_all(tsdn, &arena->pa_shard.pac, decay, decay_stats, ecache, /* fully_decay */ all);
    ...
    pac_purge_eagerness_t eagerness = arena_decide_unforced_purge_eagerness(is_background_thread);
    bool epoch_advanced = pac_maybe_decay_purge(tsdn, &arena->pa_shard.pac, decay, decay_stats, ecache, eagerness);
    size_t npages_new;
    if (epoch_advanced) {
...
static bool arena_decay_dirty(tsdn_t *tsdn, arena_t *arena, bool is_background_thread, bool all) {
    return arena_decay_impl(
        tsdn, arena, &arena->pa_shard.pac.decay_dirty, &arena->pa_shard.pac.stats->decay_dirty, &arena->pa_shard.pac.ecache_dirty,
...
static bool arena_decay_muzzy(tsdn_t *tsdn, arena_t *arena, bool is_background_thread, bool all) {
    ...
    return arena_decay_impl(
        tsdn, arena, &arena->pa_shard.pac.decay_muzzy, &arena->pa_shard.pac.stats->decay_muzzy, &arena->pa_shard.pac.ecache_muzzy,
        is_background_thread, all
    );
}
...
static bool arena_should_decay_early(
    tsdn_t *tsdn, arena_t *arena, decay_t *decay, background_thread_info_t *info, nstime_t *remaining_sleep, size_t npages_new
) {
    ...
    if (npages_new > 0) {
        uint64_t npurge_new = decay_npages_purge_in(decay, remaining_sleep, npages_new);
...
static void arena_maybe_do_deferred_work(tsdn_t *tsdn, arena_t *arena, decay_t *decay, size_t npages_new) {
    ...
    } else if (arena_should_decay_early(tsdn, arena, decay, info, &remaining_sleep, npages_new)) {
...
static void arena_bin_slabs_nonfull_insert(bin_t *bin, edata_t *slab) {
    assert(edata_nfree_get(slab) > 0);
...
static void arena_bin_slabs_nonfull_remove(bin_t *bin, edata_t *slab) {
...
static void arena_bin_slabs_full_insert(arena_t *arena, bin_t *bin, edata_t *slab) {
    assert(edata_nfree_get(slab) == 0);
    /*
...
static void arena_bin_slabs_full_remove(arena_t *arena, bin_t *bin, edata_t *slab) {
    if (arena_is_auto(arena)) {
        return;
    }
...
static void arena_bin_reset(tsd_t *tsd, arena_t *arena, bin_t *bin) {
    ...
    for (slab = edata_list_active_first(&bin->slabs_full); slab != NULL; slab = edata_list_active_first(&bin->slabs_full)) {
...
void arena_reset(tsd_t *tsd, arena_t *arena) {
    ...
    for (edata_t *edata = edata_list_active_first(&arena->large); edata != NULL; edata = edata_list_active_first(&arena->large)) {
...
static void arena_prepare_base_deletion_sync_finish(tsd_t *tsd, malloc_mutex_t **mutexes, unsigned n_mtx) {
...
static void arena_prepare_base_deletion_sync(tsd_t *tsd, malloc_mutex_t *mtx, malloc_mutex_t **delayed_mtx, unsigned *n_delayed) {
...
static void arena_prepare_base_deletion(tsd_t *tsd, base_t *base_to_destroy) {
    /*
    ...
    for (unsigned i = 0; i < total; i++) {
        ...
        arena_prepare_base_deletion_sync(tsd, &pac->ecache_dirty.mtx, delayed_mtx, &n_delayed);
        arena_prepare_base_deletion_sync(tsd, &pac->ecache_muzzy.mtx, delayed_mtx, &n_delayed);
        arena_prepare_base_deletion_sync(tsd, &pac->ecache_retained.mtx, delayed_mtx, &n_delayed);
    }
...
static edata_t *arena_slab_alloc(tsdn_t *tsdn, arena_t *arena, szind_t binind, unsigned binshard, const bin_info_t *bin_info) {
    bool deferred_work_generated = false;
    witness_assert_depth_to_rank(tsdn_witness_tsdp_get(tsdn), WITNESS_RANK_CORE, 0);

    bool guarded = san_slab_extent_decide_guard(tsdn, arena_get_ehooks(arena));
    edata_t *slab = pa_alloc(
        tsdn, &arena->pa_shard, bin_info->slab_size,
 ...
 */
static void arena_bin_refill_slabcur_with_fresh_slab(tsdn_t *tsdn, arena_t *arena, bin_t *bin, szind_t binind, edata_t *fresh_slab) {
...
static void *arena_bin_malloc_with_fresh_slab(tsdn_t *tsdn, arena_t *arena, bin_t *bin, szind_t binind, edata_t *fresh_slab) {
    malloc_mutex_assert_owner(tsdn, &bin->lock);
    arena_bin_refill_slabcur_with_fresh_slab(tsdn, arena, bin, binind, fresh_slab);
...
bin_t *arena_bin_choose(tsdn_t *tsdn, arena_t *arena, szind_t binind, unsigned *binshard_p) {
    unsigned binshard;
...
void arena_cache_bin_fill_small(
    tsdn_t *tsdn, arena_t *arena, cache_bin_t *cache_bin, cache_bin_info_t *cache_bin_info, szind_t binind, const unsigned nfill
) {
...
label_refill:
    ...
    while (filled < nfill) {
        ...
        if (slabcur != NULL && edata_nfree_get(slabcur) > 0) {
            ...
            arena_slab_reg_alloc_batch(slabcur, bin_info, cnt, &ptrs.ptr[filled]);
            made_progress = true;
            filled += cnt;
            continue;
        }
...
size_t arena_fill_small_fresh(tsdn_t *tsdn, arena_t *arena, szind_t binind, void **ptrs, size_t nfill, bool zero) {
    ...
    while (filled < nfill && (slab = arena_slab_alloc(tsdn, arena, binind, binshard, bin_info)) != NULL) {
        ...
        assert(batch > 0);
        arena_slab_reg_alloc_batch(slab, bin_info, (unsigned)batch, &ptrs[filled]);
 ...
 */
static void *arena_bin_malloc_no_fresh_slab(tsdn_t *tsdn, arena_t *arena, bin_t *bin, szind_t binind) {
    malloc_mutex_assert_owner(tsdn, &bin->lock);
...
static size_t arena_prof_demote(tsdn_t *tsdn, edata_t *edata, const void *ptr) {
    cassert(config_prof);
    assert(ptr != NULL);
...
static void arena_dissociate_bin_slab(arena_t *arena, edata_t *slab, bin_t *bin) {
...
static void arena_dalloc_bin_slab_prepare(tsdn_t *tsdn, edata_t *slab, bin_t *bin) {
    malloc_mutex_assert_owner(tsdn, &bin->lock);
...
void arena_dalloc_bin_locked_handle_newly_empty(tsdn_t *tsdn, arena_t *arena, edata_t *slab, bin_t *bin) {
    arena_dissociate_bin_slab(arena, slab, bin);
    arena_dalloc_bin_slab_prepare(tsdn, slab, bin);
}

void arena_dalloc_bin_locked_handle_newly_nonempty(tsdn_t *tsdn, arena_t *arena, edata_t *slab, bin_t *bin) {
    arena_bin_slabs_full_remove(arena, bin, slab);
    arena_bin_lower_slab(tsdn, arena, slab, bin);
}

static void arena_dalloc_bin(tsdn_t *tsdn, arena_t *arena, edata_t *edata, void *ptr) {
    ...
    bool ret = arena_dalloc_bin_locked_step(tsdn, arena, bin, &info, binind, edata, ptr);
...
void *arena_ralloc(
    tsdn_t *tsdn, arena_t *arena, void *ptr, size_t oldsize, size_t size, size_t alignment, bool zero, tcache_t *tcache, hook_ralloc_args_t *hook_args
) {
    ...
    if (likely(usize <= SC_SMALL_MAXCLASS)) {
        ...
        if (!arena_ralloc_no_move(tsdn, ptr, oldsize, usize, 0, zero, &newsize)) {
            hook_invoke_expand(
                hook_args->is_realloc ? hook_expand_realloc : hook_expand_rallocx, ptr, oldsize, usize, (uintptr_t)ptr, hook_args->args
            );
            return ptr;
        }
    }
...
extent_hooks_t *arena_set_extent_hooks(tsd_t *tsd, arena_t *arena, extent_hooks_t *extent_hooks) {
...
bool arena_retain_grow_limit_get_set(tsd_t *tsd, arena_t *arena, size_t *old_limit, size_t *new_limit) {
    assert(opt_retain);
    return pac_retain_grow_limit_get_set(tsd_tsdn(tsd), &arena->pa_shard.pac, old_limit, new_limit);
}
...
arena_t *arena_new(tsdn_t *tsdn, unsigned ind, const arena_config_t *config) {
    ...
    if (ind == 0) {
        ...
        base = base_new(tsdn, ind, config->extent_hooks, config->metadata_use_hooks);
        if (base == NULL) {
            return NULL;
        }
    }
    ...
    if (pa_shard_init(
            tsdn, &arena->pa_shard, &arena_pa_central_global, &arena_emap_global, base, ind, &arena->stats.pa_shard_stats,
            LOCKEDINT_MTX(arena->stats.mtx), &cur_time, oversize_threshold, arena_dirty_decay_ms_default_get(), arena_muzzy_decay_ms_default_get()
        )) {
        goto label_error;
    }
    ...
    if (opt_hpa && ehooks_are_default(base_ehooks_get(base)) && ind != 0) {
        ...
        if (pa_shard_enable_hpa(tsdn, &arena->pa_shard, &hpa_shard_opts, &opt_hpa_sec_opts)) {
            goto label_error;
        }
    }
...
arena_t *arena_choose_huge(tsd_t *tsd) {
    ...
    if (huge_arena == NULL) {
        ...
        if (arena_dirty_decay_ms_default_get() > 0) {
            arena_decay_ms_set(tsd_tsdn(tsd), huge_arena, extent_state_dirty, 0);
        }
        if (arena_muzzy_decay_ms_default_get() > 0) {
            arena_decay_ms_set(tsd_tsdn(tsd), huge_arena, extent_state_muzzy, 0);
        }
    }
...
bool arena_boot(sc_data_t *sc_data, base_t *base, bool hpa) {
    ...
    return pa_central_init(&arena_pa_central_global, base, hpa, &hpa_hooks_default);
}
...
void arena_postfork_child(tsdn_t *tsdn, arena_t *arena) {
    ...
    if (config_stats) {
        ...
        if (tcache_slow != NULL && tcache_slow->arena == arena) {
            ...
            cache_bin_array_descriptor_init(&tcache_slow->cache_bin_array_descriptor, tcache->bins);
            ql_tail_insert(&arena->cache_bin_array_descriptor_ql, &tcache_slow->cache_bin_array_descriptor, link);
        }
    }
