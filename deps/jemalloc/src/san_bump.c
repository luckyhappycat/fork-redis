...
static bool san_bump_grow_locked(tsdn_t *tsdn, san_bump_alloc_t *sba, pac_t *pac, ehooks_t *ehooks, size_t size);

edata_t *san_bump_alloc(tsdn_t *tsdn, san_bump_alloc_t *sba, pac_t *pac, ehooks_t *ehooks, size_t size, bool zero) {
    ...
    if (trail_size != 0) {
        edata_t *curr_reg_trail = extent_split_wrapper(
            tsdn, pac, ehooks, sba->curr_reg, guarded_size, trail_size,
    ...
    san_guard_pages(
        tsdn, ehooks, edata, pac->emap, /* left */ false,
        /* right */ true, /* remap */ true
    );

    if (extent_commit_zero(
            tsdn, ehooks, edata, /* commit */ true, zero,
...
static bool san_bump_grow_locked(tsdn_t *tsdn, san_bump_alloc_t *sba, pac_t *pac, ehooks_t *ehooks, size_t size) {
    ...
    sba->curr_reg = extent_alloc_wrapper(
        tsdn, pac, ehooks, NULL, alloc_size, PAGE, zeroed, &committed,
