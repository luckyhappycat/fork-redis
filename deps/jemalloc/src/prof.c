...
static bool prof_idump_accum_init(void) {
    cassert(config_prof);
...
prof_tdata_t *prof_tdata_reinit(tsd_t *tsd, prof_tdata_t *tdata) {
    ...
    char *thread_name = (tdata->thread_name != NULL) ? prof_thread_name_alloc(tsd, tdata->thread_name) : NULL;
...
prof_backtrace_hook_t prof_backtrace_hook_get() {
    return (prof_backtrace_hook_t)atomic_load_p(&prof_backtrace_hook, ATOMIC_ACQUIRE);
}
...
bool prof_boot2(tsd_t *tsd, base_t *base) {
    ...
    if (malloc_mutex_init(
            &prof_thread_active_init_mtx, "prof_thread_active_init", WITNESS_RANK_PROF_THREAD_ACTIVE_INIT, malloc_mutex_rank_exclusive
        )) {
        return true;
    }
    ...
    if (malloc_mutex_init(&prof_stats_mtx, "prof_stats", WITNESS_RANK_PROF_STATS, malloc_mutex_rank_exclusive)) {
        return true;
    }
    if (malloc_mutex_init(&prof_dump_filename_mtx, "prof_dump_filename", WITNESS_RANK_PROF_DUMP_FILENAME, malloc_mutex_rank_exclusive)) {
        return true;
    }
    ...
    if (opt_prof) {
        ...
        gctx_locks = (malloc_mutex_t *)base_alloc(tsd_tsdn(tsd), base, PROF_NCTX_LOCKS * sizeof(malloc_mutex_t), CACHELINE);
        if (gctx_locks == NULL) {
            return true;
        }
        ...
        tdata_locks = (malloc_mutex_t *)base_alloc(tsd_tsdn(tsd), base, PROF_NTDATA_LOCKS * sizeof(malloc_mutex_t), CACHELINE);
        if (tdata_locks == NULL) {
            return true;
        }
