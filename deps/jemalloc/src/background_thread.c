...
#define NOT_REACHED \
    { not_reached(); }
bool background_thread_create(tsd_t *tsd, unsigned arena_ind) NOT_REACHED bool background_threads_enable(tsd_t *tsd) NOT_REACHED
    bool background_threads_disable(tsd_t *tsd) NOT_REACHED bool background_thread_is_started(background_thread_info_t *info) NOT_REACHED
    void background_thread_wakeup_early(background_thread_info_t *info, nstime_t *remaining_sleep) NOT_REACHED
    void background_thread_prefork0(tsdn_t *tsdn) NOT_REACHED void background_thread_prefork1(tsdn_t *tsdn) NOT_REACHED
    void background_thread_postfork_parent(tsdn_t *tsdn) NOT_REACHED void background_thread_postfork_child(tsdn_t *tsdn) NOT_REACHED
...
static inline void background_work_sleep_once(tsdn_t *tsdn, background_thread_info_t *info, unsigned ind) {
    ...
    for (unsigned i = ind; i < narenas; i += max_background_threads) {
        ...
        uint64_t ns_arena_deferred = pa_shard_time_until_deferred_work(tsdn, &arena->pa_shard);
...
bool background_threads_enable(tsd_t *tsd) {
    ...
    for (unsigned i = 0; i < narenas; i++) {
        ...
        if (arena != NULL) {
            pa_shard_set_deferral_allowed(tsd_tsdn(tsd), &arena->pa_shard, true);
        }
    }
    return false;
}
...
bool background_threads_disable(tsd_t *tsd) {
    ...
    for (unsigned i = 0; i < narenas; i++) {
        arena_t *arena = arena_get(tsd_tsdn(tsd), i, false);
        if (arena != NULL) {
            pa_shard_set_deferral_allowed(tsd_tsdn(tsd), &arena->pa_shard, false);
        }
    }
...
void background_thread_wakeup_early(background_thread_info_t *info, nstime_t *remaining_sleep) {
    /*
...
bool background_thread_stats_read(tsdn_t *tsdn, background_thread_stats_t *stats) {
    ...
    for (unsigned i = 0; i < max_background_threads; i++) {
        ...
        if (info->state != background_thread_stopped) {
            ...
            malloc_mutex_prof_max_update(tsdn, &stats->max_counter_per_bg_thd, &info->mtx);
        }
        malloc_mutex_unlock(tsdn, &info->mtx);
    }
...
bool background_thread_boot1(tsdn_t *tsdn, base_t *base) {
    ...
    background_thread_info =
        (background_thread_info_t *)base_alloc(tsdn, base, opt_max_background_threads * sizeof(background_thread_info_t), CACHELINE);
