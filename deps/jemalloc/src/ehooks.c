 ...
 */
static void *extent_alloc_core(
    tsdn_t *tsdn, arena_t *arena, void *new_addr, size_t size, size_t alignment, bool *zero, bool *commit, dss_prec_t dss_prec
) {
    void *ret;
...
void *ehooks_default_alloc_impl(tsdn_t *tsdn, void *new_addr, size_t size, size_t alignment, bool *zero, bool *commit, unsigned arena_ind) {
    ...
    dss_prec_t dss = (arena == NULL) ? dss_prec_disabled : (dss_prec_t)atomic_load_u(&arena->dss_prec, ATOMIC_RELAXED);
    void *ret = extent_alloc_core(tsdn, arena, new_addr, size, alignment, zero, commit, dss);
...
static void *ehooks_default_alloc(
    extent_hooks_t *extent_hooks, void *new_addr, size_t size, size_t alignment, bool *zero, bool *commit, unsigned arena_ind
) {
    return ehooks_default_alloc_impl(tsdn_fetch(), new_addr, size, ALIGNMENT_CEILING(alignment, PAGE), zero, commit, arena_ind);
}
...
static bool ehooks_default_dalloc(extent_hooks_t *extent_hooks, void *addr, size_t size, bool committed, unsigned arena_ind) {
...
static void ehooks_default_destroy(extent_hooks_t *extent_hooks, void *addr, size_t size, bool committed, unsigned arena_ind) {
...
static bool ehooks_default_commit(extent_hooks_t *extent_hooks, void *addr, size_t size, size_t offset, size_t length, unsigned arena_ind) {
...
static bool ehooks_default_decommit(extent_hooks_t *extent_hooks, void *addr, size_t size, size_t offset, size_t length, unsigned arena_ind) {
...
static bool ehooks_default_purge_lazy(extent_hooks_t *extent_hooks, void *addr, size_t size, size_t offset, size_t length, unsigned arena_ind) {
    assert(addr != NULL);
...
static bool ehooks_default_purge_forced(extent_hooks_t *extent_hooks, void *addr, size_t size, size_t offset, size_t length, unsigned arena_ind) {
    assert(addr != NULL);
    assert((offset & PAGE_MASK) == 0);
    assert(length != 0);
    assert((length & PAGE_MASK) == 0);
...
static bool ehooks_default_split(
    extent_hooks_t *extent_hooks, void *addr, size_t size, size_t size_a, size_t size_b, bool committed, unsigned arena_ind
) {
...
bool ehooks_default_merge_impl(tsdn_t *tsdn, void *addr_a, void *addr_b) {
    ...
    if (config_debug) {
        ...
        assert(extent_neighbor_head_state_mergeable(
            head_a, head_b,
            /* forward */ true
        ));
    }
...
bool ehooks_default_merge(
    extent_hooks_t *extent_hooks, void *addr_a, size_t size_a, void *addr_b, size_t size_b, bool committed, unsigned arena_ind
) {
...
const extent_hooks_t ehooks_default_extent_hooks = {ehooks_default_alloc,        ehooks_default_dalloc,   ehooks_default_destroy,
                                                    ehooks_default_commit,       ehooks_default_decommit,
#ifdef PAGES_CAN_PURGE_LAZY
