...
static void psset_bin_stats_accum(psset_bin_stats_t *dst, psset_bin_stats_t *src) {
...
void psset_stats_accum(psset_stats_t *dst, psset_stats_t *src) {
    ...
    for (pszind_t i = 0; i < PSSET_NPSIZES; i++) {
        psset_bin_stats_accum(&dst->nonfull_slabs[i][0], &src->nonfull_slabs[i][0]);
        psset_bin_stats_accum(&dst->nonfull_slabs[i][1], &src->nonfull_slabs[i][1]);
    }
}
 ...
 */
JEMALLOC_ALWAYS_INLINE void psset_bin_stats_insert_remove(psset_t *psset, psset_bin_stats_t *binstats, hpdata_t *ps, bool insert) {
    ...
    if (config_debug) {
        ...
        for (size_t huge = 0; huge <= 1; huge++) {
            ...
            psset_bin_stats_accum(&check_stats, &psset->stats.empty_slabs[huge]);
            for (pszind_t pind = 0; pind < PSSET_NPSIZES; pind++) {
                psset_bin_stats_accum(&check_stats, &psset->stats.nonfull_slabs[pind][huge]);
            }
        }
...
static void psset_bin_stats_insert(psset_t *psset, psset_bin_stats_t *binstats, hpdata_t *ps) {
    psset_bin_stats_insert_remove(psset, binstats, ps, true);
}

static void psset_bin_stats_remove(psset_t *psset, psset_bin_stats_t *binstats, hpdata_t *ps) {
    psset_bin_stats_insert_remove(psset, binstats, ps, false);
}

static void psset_hpdata_heap_remove(psset_t *psset, pszind_t pind, hpdata_t *ps) {
...
static void psset_hpdata_heap_insert(psset_t *psset, pszind_t pind, hpdata_t *ps) {
    if (hpdata_age_heap_empty(&psset->pageslabs[pind])) {
...
static void psset_stats_insert(psset_t *psset, hpdata_t *ps) {
    if (hpdata_empty(ps)) {
        ...
        pszind_t pind = sz_psz2ind(sz_psz_quantize_floor(longest_free_range << LG_PAGE));
...
static void psset_stats_remove(psset_t *psset, hpdata_t *ps) {
    if (hpdata_empty(ps)) {
 ...
 * requests.
 */
static void psset_alloc_container_insert(psset_t *psset, hpdata_t *ps) {
...
static void psset_alloc_container_remove(psset_t *psset, hpdata_t *ps) {
    assert(hpdata_in_psset_alloc_container_get(ps));
...
static size_t psset_purge_list_ind(hpdata_t *ps) {
...
static void psset_maybe_remove_purge_list(psset_t *psset, hpdata_t *ps) {
    /*
...
static void psset_maybe_insert_purge_list(psset_t *psset, hpdata_t *ps) {
    if (hpdata_purge_allowed_get(ps)) {
        size_t ind = psset_purge_list_ind(ps);
        hpdata_purge_list_t *purge_list = &psset->to_purge[ind];
        if (hpdata_purge_list_empty(purge_list)) {
...
void psset_update_end(psset_t *psset, hpdata_t *ps) {
    ...
    if (hpdata_hugify_allowed_get(ps) && !hpdata_in_psset_hugify_container_get(ps)) {
        hpdata_in_psset_hugify_container_set(ps, true);
        hpdata_hugify_list_append(&psset->to_hugify, ps);
    } else if (!hpdata_hugify_allowed_get(ps) && hpdata_in_psset_hugify_container_get(ps)) {
...
hpdata_t *psset_pick_purge(psset_t *psset) {
    ssize_t ind_ssz = fb_fls(psset->purge_bitmap, PSSET_NPURGE_LISTS, PSSET_NPURGE_LISTS - 1);
    if (ind_ssz < 0) {
        return NULL;
    }
