...
#define CTL_LEAF(mib, miblen, leaf, v, t)                                           \
    do {                                                                            \
        ...
        size_t miblen_new = CTL_MAX_DEPTH;                                          \
        size_t sz = sizeof(t);                                                      \
        xmallctlbymibname(mib, miblen, leaf, &miblen_new, (void *)v, &sz, NULL, 0); \
...
static void mutex_stats_init_cols(
    emitter_row_t *row, const char *table_name, emitter_col_t *name, emitter_col_t col_uint64_t[mutex_prof_num_uint64_t_counters],
...
static void mutex_stats_read_global(
    size_t mib[], size_t miblen, const char *name, emitter_col_t *col_name, emitter_col_t col_uint64_t[mutex_prof_num_uint64_t_counters],
    emitter_col_t col_uint32_t[mutex_prof_num_uint32_t_counters], uint64_t uptime
) {
...
#define OP(counter, counter_type, human, derived, base_counter)                                    \
    ...
    if (!derived) {                                                                                \
        CTL_LEAF(mib, miblen_name, #counter, (counter_type *)&dst->bool_val, counter_type);        \
    } else {                                                                                       \
...
static void mutex_stats_read_arena_bin(
    size_t mib[], size_t miblen, emitter_col_t col_uint64_t[mutex_prof_num_uint64_t_counters],
    emitter_col_t col_uint32_t[mutex_prof_num_uint32_t_counters], uint64_t uptime
) {
...
#define OP(counter, counter_type, human, derived, base_counter)                                    \
    ...
    dst->type = EMITTER_TYPE_##counter_type;                                                       \
    if (!derived) {                                                                                \
        CTL_LEAF(mib, miblen_mutex, #counter, (counter_type *)&dst->bool_val, counter_type);       \
    } else {                                                                                       \
        emitter_col_t *base = &col_##counter_type[mutex_counter_##base_counter];                   \
        dst->counter_type##_val = (counter_type)rate_per_second(base->counter_type##_val, uptime); \
    }
...
static void mutex_stats_emit(
    emitter_t *emitter, emitter_row_t *row, emitter_col_t col_uint64_t[mutex_prof_num_uint64_t_counters],
    emitter_col_t col_uint32_t[mutex_prof_num_uint32_t_counters]
) {
    if (row != NULL) {
...
#define COL_HDR_INIT(row_name, column_name, human, left_or_right, col_width, etype) \
    COL_INIT(row_name, column_name, left_or_right, col_width, etype)                \
...
static void stats_arena_bins_print(emitter_t *emitter, bool mutex, unsigned i, uint64_t uptime) {
    ...
    bool prof_stats_on = config_prof && opt_prof && opt_prof_stats && i == MALLCTL_ARENAS_ALL;
    ...
    for (j = 0, in_gap = false; j < nbins; j++) {
        ...
        if (mutex) {
            mutex_stats_read_arena_bin(stats_arenas_mib, 5, col_mutex64, col_mutex32, uptime);
        }
        ...
        if (prof_stats_on) {
            emitter_json_kv(emitter, "prof_live_requested", emitter_type_uint64, &prof_live.req_sum);
            emitter_json_kv(emitter, "prof_live_count", emitter_type_uint64, &prof_live.count);
            emitter_json_kv(emitter, "prof_accum_requested", emitter_type_uint64, &prof_accum.req_sum);
            emitter_json_kv(emitter, "prof_accum_count", emitter_type_uint64, &prof_accum.count);
        }
...
static void stats_arena_extents_print(emitter_t *emitter, unsigned i) {
    ...
    for (j = 0; j < SC_NPSIZES; j++) {
        ...
        CTL_LEAF(stats_arenas_mib, 5, "retained_bytes", &retained_bytes, size_t);
...
static void stats_arena_hpa_shard_print(emitter_t *emitter, unsigned i, uint64_t uptime) {
    emitter_row_t header_row;
    emitter_row_init(&header_row);
    emitter_row_t row;
    emitter_row_init(&row);
    ...
    CTL_M2_GET("stats.arenas.0.hpa_shard.npurge_passes", i, &npurge_passes, uint64_t);
    ...
    CTL_M2_GET("stats.arenas.0.hpa_shard.ndehugifies", i, &ndehugifies, uint64_t);
    ...
    emitter_kv(emitter, "sec_bytes", "Bytes in small extent cache", emitter_type_size, &sec_bytes);
    ...
    emitter_table_printf(
        ...
        " / sec)\n"
        "\n",
        npurge_passes, rate_per_second(npurge_passes, uptime), npurges, rate_per_second(npurges, uptime), nhugifies,
        rate_per_second(nhugifies, uptime), ndehugifies, rate_per_second(ndehugifies, uptime)
    );

    emitter_json_object_kv_begin(emitter, "hpa_shard");
    emitter_json_kv(emitter, "npurge_passes", emitter_type_uint64, &npurge_passes);
    ...
    CTL_M2_GET("stats.arenas.0.hpa_shard.full_slabs.npageslabs_huge", i, &npageslabs_huge, size_t);
    CTL_M2_GET("stats.arenas.0.hpa_shard.full_slabs.nactive_huge", i, &nactive_huge, size_t);
    CTL_M2_GET("stats.arenas.0.hpa_shard.full_slabs.ndirty_huge", i, &ndirty_huge, size_t);

    CTL_M2_GET("stats.arenas.0.hpa_shard.full_slabs.npageslabs_nonhuge", i, &npageslabs_nonhuge, size_t);
    CTL_M2_GET("stats.arenas.0.hpa_shard.full_slabs.nactive_nonhuge", i, &nactive_nonhuge, size_t);
    CTL_M2_GET("stats.arenas.0.hpa_shard.full_slabs.ndirty_nonhuge", i, &ndirty_nonhuge, size_t);
    ...
    emitter_table_printf(
        ...
        npageslabs_huge, npageslabs_nonhuge, nactive_huge, nactive_nonhuge, ndirty_huge, ndirty_nonhuge, nretained_nonhuge
    );

    emitter_json_object_kv_begin(emitter, "full_slabs");
    emitter_json_kv(emitter, "npageslabs_huge", emitter_type_size, &npageslabs_huge);
    emitter_json_kv(emitter, "nactive_huge", emitter_type_size, &nactive_huge);
    emitter_json_kv(emitter, "nactive_huge", emitter_type_size, &nactive_huge);
    emitter_json_kv(emitter, "npageslabs_nonhuge", emitter_type_size, &npageslabs_nonhuge);
    emitter_json_kv(emitter, "nactive_nonhuge", emitter_type_size, &nactive_nonhuge);
    emitter_json_kv(emitter, "ndirty_nonhuge", emitter_type_size, &ndirty_nonhuge);
    emitter_json_object_end(emitter); /* End "full_slabs" */

    /* Next, empty slab stats. */
    CTL_M2_GET("stats.arenas.0.hpa_shard.empty_slabs.npageslabs_huge", i, &npageslabs_huge, size_t);
    CTL_M2_GET("stats.arenas.0.hpa_shard.empty_slabs.nactive_huge", i, &nactive_huge, size_t);
    CTL_M2_GET("stats.arenas.0.hpa_shard.empty_slabs.ndirty_huge", i, &ndirty_huge, size_t);

    CTL_M2_GET("stats.arenas.0.hpa_shard.empty_slabs.npageslabs_nonhuge", i, &npageslabs_nonhuge, size_t);
    CTL_M2_GET("stats.arenas.0.hpa_shard.empty_slabs.nactive_nonhuge", i, &nactive_nonhuge, size_t);
    CTL_M2_GET("stats.arenas.0.hpa_shard.empty_slabs.ndirty_nonhuge", i, &ndirty_nonhuge, size_t);
    nretained_nonhuge = npageslabs_nonhuge * HUGEPAGE_PAGES - nactive_nonhuge - ndirty_nonhuge;
    ...
    for (pszind_t j = 0; j < PSSET_NPSIZES && j < SC_NPSIZES; j++) {
        ...
        CTL_LEAF(stats_arenas_mib, 6, "npageslabs_huge", &npageslabs_huge, size_t);
        CTL_LEAF(stats_arenas_mib, 6, "nactive_huge", &nactive_huge, size_t);
        CTL_LEAF(stats_arenas_mib, 6, "ndirty_huge", &ndirty_huge, size_t);

        CTL_LEAF(stats_arenas_mib, 6, "npageslabs_nonhuge", &npageslabs_nonhuge, size_t);
        CTL_LEAF(stats_arenas_mib, 6, "nactive_nonhuge", &nactive_nonhuge, size_t);
        CTL_LEAF(stats_arenas_mib, 6, "ndirty_nonhuge", &ndirty_nonhuge, size_t);
        nretained_nonhuge = npageslabs_nonhuge * HUGEPAGE_PAGES - nactive_nonhuge - ndirty_nonhuge;
        ...
        emitter_json_kv(emitter, "npageslabs_huge", emitter_type_size, &npageslabs_huge);
        emitter_json_kv(emitter, "nactive_huge", emitter_type_size, &nactive_huge);
        emitter_json_kv(emitter, "ndirty_huge", emitter_type_size, &ndirty_huge);
        emitter_json_kv(emitter, "npageslabs_nonhuge", emitter_type_size, &npageslabs_nonhuge);
        emitter_json_kv(emitter, "nactive_nonhuge", emitter_type_size, &nactive_nonhuge);
        emitter_json_kv(emitter, "ndirty_nonhuge", emitter_type_size, &ndirty_nonhuge);
        emitter_json_object_end(emitter);
...
static void stats_arena_mutexes_print(emitter_t *emitter, unsigned arena_ind, uint64_t uptime) {
    ...
    for (mutex_prof_arena_ind_t i = 0; i < mutex_prof_num_arena_mutexes; i++) {
        ...
        mutex_stats_read_arena(stats_arenas_mib, 4, name, &col_name, col64, col32, uptime);
...
JEMALLOC_COLD
static void stats_arena_print(emitter_t *emitter, unsigned i, bool bins, bool large, bool mutex, bool extents, bool hpa) {
    unsigned nthreads;
    const char *dss;
...
#define OPT_WRITE_CHAR_P(name) OPT_WRITE(name, cpv, cpsz, emitter_type_string)
    ...
    if (je_mallctl("opt.hpa_dirty_mult", (void *)&u32v, &u32sz, NULL, 0) == 0) {
        ...
        if (u32v == (uint32_t)-1) {
            const char *neg1 = "-1";
            emitter_kv(emitter, "hpa_dirty_mult", "opt.hpa_dirty_mult", emitter_type_string, &neg1);
        } else {
            char buf[FXP_BUF_SIZE];
            fxp_print(u32v, buf);
            const char *bufp = buf;
            emitter_kv(emitter, "hpa_dirty_mult", "opt.hpa_dirty_mult", emitter_type_string, &bufp);
        }
    }
...
#undef OPT_WRITE_CHAR_P
    ...
    emitter_kv(emitter, "nhbins", "Number of thread-cache bin size classes", emitter_type_unsigned, &arenas_nhbins);
...
JEMALLOC_COLD
static void stats_print_helper(
    emitter_t *emitter, bool merged, bool destroyed, bool unmerged, bool bins, bool large, bool mutex, bool extents, bool hpa
) {
    /*
    ...
    if (mutex) {
        ...
        for (int i = 0; i < mutex_prof_num_global_mutexes; i++) {
            mutex_stats_read_global(stats_mutexes_mib, 2, global_mutex_names[i], &name, col64, col32, uptime);
    ...
    if (merged || destroyed || unmerged) {
        ...
        if (merged && (ninitialized > 1 || !unmerged)) {
            ...
            stats_arena_print(emitter, MALLCTL_ARENAS_ALL, bins, large, mutex, extents, hpa);
        ...
        if (destroyed_initialized && destroyed) {
            ...
            stats_arena_print(emitter, MALLCTL_ARENAS_DESTROYED, bins, large, mutex, extents, hpa);
        ...
        if (unmerged) {
            for (i = j = 0; i < narenas; i++) {
                if (initialized[i]) {
                    ...
                    stats_arena_print(emitter, i, bins, large, mutex, extents, hpa);
                    /* Close "<arena-ind>". */
                    emitter_json_object_end(emitter);
                }
            }
...
#undef OPTION
    ...
    emitter_t emitter;
    emitter_init(&emitter, json ? emitter_output_json_compact : emitter_output_table, write_cb, cbopaque);
