...
static size_t hpa_alloc_batch(tsdn_t *tsdn, pai_t *self, size_t size, size_t nallocs, edata_list_active_t *results, bool *deferred_work_generated);
static bool hpa_expand(tsdn_t *tsdn, pai_t *self, edata_t *edata, size_t old_size, size_t new_size, bool zero, bool *deferred_work_generated);
static bool hpa_shrink(tsdn_t *tsdn, pai_t *self, edata_t *edata, size_t old_size, size_t new_size, bool *deferred_work_generated);
static void hpa_dalloc(tsdn_t *tsdn, pai_t *self, edata_t *edata, bool *deferred_work_generated);
static void hpa_dalloc_batch(tsdn_t *tsdn, pai_t *self, edata_list_active_t *list, bool *deferred_work_generated);
...
static void hpa_do_consistency_checks(hpa_shard_t *shard) {
...
bool hpa_central_init(hpa_central_t *central, base_t *base, const hpa_hooks_t *hooks) {
    ...
    bool err;
    err = malloc_mutex_init(&central->grow_mtx, "hpa_central_grow", WITNESS_RANK_HPA_CENTRAL_GROW, malloc_mutex_rank_exclusive);
    if (err) {
        return true;
    }
    err = malloc_mutex_init(&central->mtx, "hpa_central", WITNESS_RANK_HPA_CENTRAL, malloc_mutex_rank_exclusive);
    if (err) {
        return true;
    }
    central->base = base;
...
static hpdata_t *hpa_alloc_ps(tsdn_t *tsdn, hpa_central_t *central) {
    return (hpdata_t *)base_alloc(tsdn, central->base, sizeof(hpdata_t), CACHELINE);
}

hpdata_t *hpa_central_extract(tsdn_t *tsdn, hpa_central_t *central, size_t size, bool *oom) {
     ...
     */
    witness_assert_positive_depth_to_rank(tsdn_witness_tsdp_get(tsdn), WITNESS_RANK_HPA_SHARD_GROW);
...
bool hpa_shard_init(
    hpa_shard_t *shard, hpa_central_t *central, emap_t *emap, base_t *base, edata_cache_t *edata_cache, unsigned ind, const hpa_shard_opts_t *opts
) {
    /* malloc_conf processing should have filtered out these cases. */
    assert(hpa_supported());
    bool err;
    err = malloc_mutex_init(&shard->grow_mtx, "hpa_shard_grow", WITNESS_RANK_HPA_SHARD_GROW, malloc_mutex_rank_exclusive);
    if (err) {
        return true;
    }
    err = malloc_mutex_init(&shard->mtx, "hpa_shard", WITNESS_RANK_HPA_SHARD, malloc_mutex_rank_exclusive);
    if (err) {
        return true;
    }
 ...
 * locking here.
 */
static void hpa_shard_nonderived_stats_accum(hpa_shard_nonderived_stats_t *dst, hpa_shard_nonderived_stats_t *src) {
...
void hpa_shard_stats_accum(hpa_shard_stats_t *dst, hpa_shard_stats_t *src) {
    ...
    hpa_shard_nonderived_stats_accum(&dst->nonderived_stats, &src->nonderived_stats);
}

void hpa_shard_stats_merge(tsdn_t *tsdn, hpa_shard_t *shard, hpa_shard_stats_t *dst) {
    hpa_do_consistency_checks(shard);
...
static bool hpa_good_hugification_candidate(hpa_shard_t *shard, hpdata_t *ps) {
    /*
...
static size_t hpa_ndirty_max(tsdn_t *tsdn, hpa_shard_t *shard) {
    malloc_mutex_assert_owner(tsdn, &shard->mtx);
...
static bool hpa_hugify_blocked_by_ndirty(tsdn_t *tsdn, hpa_shard_t *shard) {
    malloc_mutex_assert_owner(tsdn, &shard->mtx);
    hpdata_t *to_hugify = psset_pick_hugify(&shard->psset);
    if (to_hugify == NULL) {
        return false;
    }
    return hpa_adjusted_ndirty(tsdn, shard) + hpdata_nretained_get(to_hugify) > hpa_ndirty_max(tsdn, shard);
}
...
static void hpa_update_purge_hugify_eligibility(tsdn_t *tsdn, hpa_shard_t *shard, hpdata_t *ps) {
    malloc_mutex_assert_owner(tsdn, &shard->mtx);
...
static bool hpa_shard_has_deferred_work(tsdn_t *tsdn, hpa_shard_t *shard) {
    malloc_mutex_assert_owner(tsdn, &shard->mtx);
    hpdata_t *to_hugify = psset_pick_hugify(&shard->psset);
...
static bool hpa_try_purge(tsdn_t *tsdn, hpa_shard_t *shard) {
    malloc_mutex_assert_owner(tsdn, &shard->mtx);
    ...
    void *purge_addr;
    size_t purge_size;
    while (hpdata_purge_next(to_purge, &purge_state, &purge_addr, &purge_size)) {
...
static bool hpa_try_hugify(tsdn_t *tsdn, hpa_shard_t *shard) {
    malloc_mutex_assert_owner(tsdn, &shard->mtx);
 ...
 */
static void hpa_shard_maybe_do_deferred_work(tsdn_t *tsdn, hpa_shard_t *shard, bool forced) {
    malloc_mutex_assert_owner(tsdn, &shard->mtx);
...
static edata_t *hpa_try_alloc_one_no_grow(tsdn_t *tsdn, hpa_shard_t *shard, size_t size, bool *oom) {
    bool err;
    ...
    edata_init(
        edata, shard->ind, addr, size, /* slab */ false, SC_NSIZES, /* sn */ hpdata_age_get(ps), extent_state_active,
        /* zeroed */ false, /* committed */ true, EXTENT_PAI_HPA, EXTENT_NOT_HEAD
    );
...
static size_t hpa_try_alloc_batch_no_grow(
    tsdn_t *tsdn, hpa_shard_t *shard, size_t size, bool *oom, size_t nallocs, edata_list_active_t *results, bool *deferred_work_generated
) {
    malloc_mutex_lock(tsdn, &shard->mtx);
    size_t nsuccess = 0;
...
static size_t hpa_alloc_batch_psset(
    tsdn_t *tsdn, hpa_shard_t *shard, size_t size, size_t nallocs, edata_list_active_t *results, bool *deferred_work_generated
) {
    assert(size <= shard->opts.slab_max_alloc);
    bool oom = false;

    size_t nsuccess = hpa_try_alloc_batch_no_grow(tsdn, shard, size, &oom, nallocs, results, deferred_work_generated);
     ...
     */
    nsuccess += hpa_try_alloc_batch_no_grow(tsdn, shard, size, &oom, nallocs - nsuccess, results, deferred_work_generated);
    if (nsuccess == nallocs || oom) {
        malloc_mutex_unlock(tsdn, &shard->grow_mtx);
        return nsuccess;
    }
...
static hpa_shard_t *hpa_from_pai(pai_t *self) {
...
static edata_t *hpa_alloc(
    ...
    size_t nallocs = hpa_alloc_batch(tsdn, self, size, /* nallocs */ 1, &results, deferred_work_generated);
...
static void hpa_dalloc_prepare_unlocked(tsdn_t *tsdn, hpa_shard_t *shard, edata_t *edata) {
...
static void hpa_dalloc_locked(tsdn_t *tsdn, hpa_shard_t *shard, edata_t *edata) {
    malloc_mutex_assert_owner(tsdn, &shard->mtx);
...
static uint64_t hpa_time_until_deferred_work(tsdn_t *tsdn, pai_t *self) {
    ...
    if (to_hugify != NULL) {
        nstime_t time_hugify_allowed = hpdata_time_hugify_allowed(to_hugify);
        uint64_t since_hugify_allowed_ms = shard->central->hooks.ms_since(&time_hugify_allowed);
        /*
    ...
    if (hpa_should_purge(tsdn, shard)) {
        ...
        uint64_t since_last_purge_ms = shard->central->hooks.ms_since(&shard->last_purge);

        if (since_last_purge_ms < shard->opts.min_purge_interval_ms) {
            uint64_t until_purge_ns;
            until_purge_ns = shard->opts.min_purge_interval_ms - since_last_purge_ms;
...
static void hpa_assert_empty(tsdn_t *tsdn, hpa_shard_t *shard, psset_t *psset) {
    malloc_mutex_assert_owner(tsdn, &shard->mtx);
...
void hpa_shard_set_deferral_allowed(tsdn_t *tsdn, hpa_shard_t *shard, bool deferral_allowed) {
    hpa_do_consistency_checks(shard);
