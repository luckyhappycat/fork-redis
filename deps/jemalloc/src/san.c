...
static inline void san_find_guarded_addr(edata_t *edata, uintptr_t *guard1, uintptr_t *guard2, uintptr_t *addr, size_t size, bool left, bool right) {
...
static inline void san_find_unguarded_addr(
    edata_t *edata, uintptr_t *guard1, uintptr_t *guard2, uintptr_t *addr, size_t size, bool left, bool right
) {
    assert(edata_guarded_get(edata));
    assert(size % PAGE == 0);
    *addr = (uintptr_t)edata_base_get(edata);
...
void san_guard_pages(tsdn_t *tsdn, ehooks_t *ehooks, edata_t *edata, emap_t *emap, bool left, bool right, bool remap) {
    ...
    size_t usize = (left && right) ? san_two_side_unguarded_sz(size_with_guards) : san_one_side_unguarded_sz(size_with_guards);
    ...
    if (remap) {
        emap_register_boundary(
            tsdn, emap, edata, SC_NSIZES,
            /* slab */ false
        );
    }
}

static void san_unguard_pages_impl(tsdn_t *tsdn, ehooks_t *ehooks, edata_t *edata, emap_t *emap, bool left, bool right, bool remap) {
    assert(left || right);
    ...
    size_t size_with_guards = (left && right) ? san_two_side_guarded_sz(size) : san_one_side_guarded_sz(size);
...
void san_unguard_pages(tsdn_t *tsdn, ehooks_t *ehooks, edata_t *edata, emap_t *emap, bool left, bool right) {
    san_unguard_pages_impl(
        tsdn, ehooks, edata, emap, left, right,
        /* remap */ true
    );
}
...
void san_unguard_pages_pre_destroy(tsdn_t *tsdn, ehooks_t *ehooks, edata_t *edata, emap_t *emap) {
     ...
     */
    san_unguard_pages_impl(
        tsdn, ehooks, edata, emap, /* left */ false,
        /* right */ true, /* remap */ false
    );
}

static bool san_stashed_corrupted(void *ptr, size_t size) {
