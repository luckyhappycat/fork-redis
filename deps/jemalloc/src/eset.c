...
static void eset_bin_init(eset_bin_t *bin) {
...
static void eset_bin_stats_init(eset_bin_stats_t *bin_stats) {
...
size_t eset_nextents_get(eset_t *eset, pszind_t pind) {
    return atomic_load_zu(&eset->bin_stats[pind].nextents, ATOMIC_RELAXED);
}

size_t eset_nbytes_get(eset_t *eset, pszind_t pind) {
    return atomic_load_zu(&eset->bin_stats[pind].nbytes, ATOMIC_RELAXED);
}

static void eset_stats_add(eset_t *eset, pszind_t pind, size_t sz) {
    size_t cur = atomic_load_zu(&eset->bin_stats[pind].nextents, ATOMIC_RELAXED);
...
static void eset_stats_sub(eset_t *eset, pszind_t pind, size_t sz) {
    size_t cur = atomic_load_zu(&eset->bin_stats[pind].nextents, ATOMIC_RELAXED);
    atomic_store_zu(&eset->bin_stats[pind].nextents, cur - 1, ATOMIC_RELAXED);
    cur = atomic_load_zu(&eset->bin_stats[pind].nbytes, ATOMIC_RELAXED);
    atomic_store_zu(&eset->bin_stats[pind].nbytes, cur - sz, ATOMIC_RELAXED);
...
void eset_insert(eset_t *eset, edata_t *edata) {
    ...
    if (edata_heap_empty(&eset->bins[pind].heap)) {
         ...
         */
        if (edata_cmp_summary_comp(edata_cmp_summary, eset->bins[pind].heap_min) < 0) {
            eset->bins[pind].heap_min = edata_cmp_summary;
        }
    }
...
void eset_remove(eset_t *eset, edata_t *edata) {
    assert(edata_state_get(edata) == eset->state || edata_state_in_transition(edata_state_get(edata)));
    ...
    if (edata_heap_empty(&eset->bins[pind].heap)) {
         ...
         */
        if (edata_cmp_summary_comp(edata_cmp_summary, eset->bins[pind].heap_min) == 0) {
            eset->bins[pind].heap_min = edata_cmp_summary_get(edata_heap_first(&eset->bins[pind].heap));
        }
    }
    ...
    atomic_store_zu(&eset->npages, cur_extents_npages - (size >> LG_PAGE), ATOMIC_RELAXED);
}
 ...
 */
static edata_t *eset_fit_alignment(eset_t *eset, size_t min_size, size_t max_size, size_t alignment) {
    pszind_t pind = sz_psz2ind(sz_psz_quantize_ceil(min_size));
    pszind_t pind_max = sz_psz2ind(sz_psz_quantize_ceil(max_size));

    for (pszind_t i = (pszind_t)fb_ffs(eset->bitmap, ESET_NPSIZES, (size_t)pind); i < pind_max;
 ...
 * for others.
 */
static edata_t *eset_first_fit(eset_t *eset, size_t size, bool exact_only, unsigned lg_max_fit) {
    edata_t *ret = NULL;
    ...
    if (exact_only) {
        return edata_heap_empty(&eset->bins[pind].heap) ? NULL : edata_heap_first(&eset->bins[pind].heap);
    }

    for (pszind_t i = (pszind_t)fb_ffs(eset->bitmap, ESET_NPSIZES, (size_t)pind); i < ESET_NPSIZES;
         i = (pszind_t)fb_ffs(eset->bitmap, ESET_NPSIZES, (size_t)i + 1)) {
        assert(!edata_heap_empty(&eset->bins[i].heap));
        ...
        if (ret == NULL || edata_cmp_summary_comp(eset->bins[i].heap_min, ret_summ) < 0) {
            /*
            ...
            assert(ret == NULL || edata_cmp_summary_comp(eset->bins[i].heap_min, edata_cmp_summary_get(edata)) == 0);
            ret = edata;
...
edata_t *eset_fit(eset_t *eset, size_t esize, size_t alignment, bool exact_only, unsigned lg_max_fit) {
