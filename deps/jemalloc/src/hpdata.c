...
static int hpdata_age_comp(const hpdata_t *a, const hpdata_t *b) {
...
void *hpdata_reserve_alloc(hpdata_t *hpdata, size_t sz) {
    ...
    while (true) {
        bool found = fb_urange_iter(hpdata->active_pages, HUGEPAGE_PAGES, start, &begin, &len);
        /*
     ...
     * count if so.
     */
    size_t new_dirty = fb_ucount(hpdata->touched_pages, HUGEPAGE_PAGES, result, npages);
...
void hpdata_unreserve(hpdata_t *hpdata, void *addr, size_t sz) {
    ...
    assert((sz & PAGE_MASK) == 0);
    size_t begin = ((uintptr_t)addr - (uintptr_t)hpdata_addr_get(hpdata)) >> LG_PAGE;
    ...
    size_t new_begin = (fb_fls(hpdata->active_pages, HUGEPAGE_PAGES, begin) + 1);
    size_t new_end = fb_ffs(hpdata->active_pages, HUGEPAGE_PAGES, begin + npages - 1);
...
size_t hpdata_purge_begin(hpdata_t *hpdata, hpdata_purge_state_t *purge_state) {
    ...
    while (next_bit < HUGEPAGE_PAGES) {
        ...
        fb_set_range(purge_state->to_purge, HUGEPAGE_PAGES, next_dirty, last_dirty - next_dirty + 1);
    ...
    assert(ndirty <= fb_scount(purge_state->to_purge, HUGEPAGE_PAGES, 0, HUGEPAGE_PAGES));
...
bool hpdata_purge_next(hpdata_t *hpdata, hpdata_purge_state_t *purge_state, void **r_purge_addr, size_t *r_purge_size) {
    ...
    size_t purge_begin;
    size_t purge_len;
    bool found_range = fb_srange_iter(purge_state->to_purge, HUGEPAGE_PAGES, purge_state->next_purge_search_begin, &purge_begin, &purge_len);
    if (!found_range) {
        return false;
    }

    *r_purge_addr = (void *)((uintptr_t)hpdata_addr_get(hpdata) + purge_begin * PAGE);
...
void hpdata_purge_end(hpdata_t *hpdata, hpdata_purge_state_t *purge_state) {
    ...
    assert(purge_state->npurged == fb_scount(purge_state->to_purge, HUGEPAGE_PAGES, 0, HUGEPAGE_PAGES));
    ...
    fb_bit_and(hpdata->touched_pages, hpdata->touched_pages, purge_state->to_purge, HUGEPAGE_PAGES);
