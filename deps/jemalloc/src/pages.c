...
#elif defined(JEMALLOC_PURGE_POSIX_MADVISE_DONTNEED) && !defined(JEMALLOC_PURGE_POSIX_MADVISE_DONTNEED_ZEROS)
...
#elif defined(JEMALLOC_PURGE_POSIX_MADVISE_DONTNEED) && defined(JEMALLOC_PURGE_POSIX_MADVISE_DONTNEED_ZEROS)
    return (unlikely(madvise_dont_need_zeros_is_faulty) || posix_madvise(addr, size, POSIX_MADV_DONTNEED) != 0);
...
#elif defined(JEMALLOC_USE_SYSCALL) && defined(SYS_openat)
    int fd = (int)syscall(SYS_openat, AT_FDCWD, "/sys/kernel/mm/transparent_hugepage/enabled", O_RDONLY);
#else
