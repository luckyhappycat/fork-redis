...
bool prof_data_init(tsd_t *tsd) {
    ...
    return ckh_new(tsd, &bt2gctx, PROF_CKH_MINITEMS, prof_bt_hash, prof_bt_keycomp);
}
...
/* Used in unit tests. */
static prof_tdata_t *prof_tdata_count_iter(prof_tdata_tree_t *tdatas_ptr, prof_tdata_t *tdata, void *arg) {
...
char *prof_thread_name_alloc(tsd_t *tsd, const char *thread_name) {
    ...
    ret = iallocztm(tsd_tsdn(tsd), size, sz_size2index(size), false, NULL, true, arena_get(TSDN_NULL, 0, true), true);
    if (ret == NULL) {
        return NULL;
    }
...
static void prof_dump_printf(write_cb_t *prof_dump_write, void *cbopaque, const char *format, ...) {
    va_list ap;
 ...
 */
#ifdef JEMALLOC_PROF
static uint64_t prof_double_uint64_cast(double d) {
    /*
 ...
 */
static void prof_do_unbias(uint64_t c_out_shifted_i, uint64_t s_out_i, uint64_t *r_c_in, uint64_t *r_s_in) {
#ifdef JEMALLOC_PROF
...
static void prof_dump_print_cnts(write_cb_t *prof_dump_write, void *cbopaque, const prof_cnt_t *cnts) {
    uint64_t curobjs;
    uint64_t curbytes;
    uint64_t accumobjs;
    uint64_t accumbytes;
    if (opt_prof_unbias) {
        prof_do_unbias(cnts->curobjs_shifted_unbiased, cnts->curbytes_unbiased, &curobjs, &curbytes);
        prof_do_unbias(cnts->accumobjs_shifted_unbiased, cnts->accumbytes_unbiased, &accumobjs, &accumbytes);
    } else {
    ...
    prof_dump_printf(prof_dump_write, cbopaque, "%" FMTu64 ": %" FMTu64 " [%" FMTu64 ": %" FMTu64 "]", curobjs, curbytes, accumobjs, accumbytes);
}
...
static void prof_tctx_merge_tdata(tsdn_t *tsdn, prof_tctx_t *tctx, prof_tdata_t *tdata) {
    ...
    switch (tctx->state) {
        ...
        case prof_tctx_state_nominal:
            ...
            tdata->cnt_summed.curobjs_shifted_unbiased += tctx->dump_cnts.curobjs_shifted_unbiased;
            tdata->cnt_summed.curbytes += tctx->dump_cnts.curbytes;
            tdata->cnt_summed.curbytes_unbiased += tctx->dump_cnts.curbytes_unbiased;
            if (opt_prof_accum) {
                tdata->cnt_summed.accumobjs += tctx->dump_cnts.accumobjs;
                tdata->cnt_summed.accumobjs_shifted_unbiased += tctx->dump_cnts.accumobjs_shifted_unbiased;
                tdata->cnt_summed.accumbytes += tctx->dump_cnts.accumbytes;
                tdata->cnt_summed.accumbytes_unbiased += tctx->dump_cnts.accumbytes_unbiased;
            }
            break;
...
static void prof_tctx_merge_gctx(tsdn_t *tsdn, prof_tctx_t *tctx, prof_gctx_t *gctx) {
    ...
    gctx->cnt_summed.curobjs_shifted_unbiased += tctx->dump_cnts.curobjs_shifted_unbiased;
    gctx->cnt_summed.curbytes += tctx->dump_cnts.curbytes;
    gctx->cnt_summed.curbytes_unbiased += tctx->dump_cnts.curbytes_unbiased;
    if (opt_prof_accum) {
        gctx->cnt_summed.accumobjs += tctx->dump_cnts.accumobjs;
        gctx->cnt_summed.accumobjs_shifted_unbiased += tctx->dump_cnts.accumobjs_shifted_unbiased;
...
static prof_tctx_t *prof_tctx_dump_iter(prof_tctx_tree_t *tctxs, prof_tctx_t *tctx, void *opaque) {
    ...
    switch (tctx->state) {
        ...
        case prof_tctx_state_dumping:
        case prof_tctx_state_purgatory:
            prof_dump_printf(arg->prof_dump_write, arg->cbopaque, "  t%" FMTu64 ": ", tctx->thr_uid);
            prof_dump_print_cnts(arg->prof_dump_write, arg->cbopaque, &tctx->dump_cnts);
...
static prof_tdata_t *prof_tdata_merge_iter(prof_tdata_tree_t *tdatas_ptr, prof_tdata_t *tdata, void *opaque) {
    ...
    if (!tdata->expired) {
        ...
        arg->cnt_all->curobjs_shifted_unbiased += tdata->cnt_summed.curobjs_shifted_unbiased;
        arg->cnt_all->curbytes += tdata->cnt_summed.curbytes;
        arg->cnt_all->curbytes_unbiased += tdata->cnt_summed.curbytes_unbiased;
        if (opt_prof_accum) {
            arg->cnt_all->accumobjs += tdata->cnt_summed.accumobjs;
            arg->cnt_all->accumobjs_shifted_unbiased += tdata->cnt_summed.accumobjs_shifted_unbiased;
            arg->cnt_all->accumbytes += tdata->cnt_summed.accumbytes;
            arg->cnt_all->accumbytes_unbiased += tdata->cnt_summed.accumbytes_unbiased;
        }
    } else {
        tdata->dumping = false;
    }
...
static prof_tdata_t *prof_tdata_dump_iter(prof_tdata_tree_t *tdatas_ptr, prof_tdata_t *tdata, void *opaque) {
    if (!tdata->dumping) {
        return NULL;
    }

    prof_dump_iter_arg_t *arg = (prof_dump_iter_arg_t *)opaque;
    prof_dump_printf(arg->prof_dump_write, arg->cbopaque, "  t%" FMTu64 ": ", tdata->thr_uid);
    prof_dump_print_cnts(arg->prof_dump_write, arg->cbopaque, &tdata->cnt_summed);
    if (tdata->thread_name != NULL) {
        arg->prof_dump_write(arg->cbopaque, " ");
...
static void prof_dump_header(prof_dump_iter_arg_t *arg, const prof_cnt_t *cnt_all) {
    prof_dump_printf(arg->prof_dump_write, arg->cbopaque, "heap_v2/%" FMTu64 "\n  t*: ", ((uint64_t)1U << lg_prof_sample));
...
static void prof_dump_gctx(prof_dump_iter_arg_t *arg, prof_gctx_t *gctx, const prof_bt_t *bt, prof_gctx_tree_t *gctxs) {
    cassert(config_prof);
    ...
    for (unsigned i = 0; i < bt->len; i++) {
        prof_dump_printf(arg->prof_dump_write, arg->cbopaque, " %#" FMTxPTR, (uintptr_t)bt->vec[i]);
    }

    arg->prof_dump_write(arg->cbopaque, "\n  t*: ");
    prof_dump_print_cnts(arg->prof_dump_write, arg->cbopaque, &gctx->cnt_summed);
    arg->prof_dump_write(arg->cbopaque, "\n");
...
static void prof_leakcheck(const prof_cnt_t *cnt_all, size_t leak_ngctx) {
    ...
    if (cnt_all->curbytes != 0) {
        ...
        malloc_printf(
            "<jemalloc>: Leak approximation summary: ~%" FMTu64 " byte%s, ~%" FMTu64 " object%s, >= %zu context%s\n", curbytes,
            (curbytes != 1) ? "s" : "", curobjs, (curobjs != 1) ? "s" : "", leak_ngctx, (leak_ngctx != 1) ? "s" : ""
        );
        malloc_printf("<jemalloc>: Run jeprof on dump output for leak detail\n");
...
static void prof_dump_prep(tsd_t *tsd, prof_tdata_t *tdata, prof_cnt_t *cnt_all, size_t *leak_ngctx, prof_gctx_tree_t *gctxs) {
    size_t tabind;
    union {
        prof_gctx_t *p;
        void *v;
    ...
    prof_tdata_merge_iter_arg_t prof_tdata_merge_iter_arg = {tsd_tsdn(tsd), cnt_all};
    malloc_mutex_lock(tsd_tsdn(tsd), &tdatas_mtx);
    tdata_tree_iter(&tdatas, NULL, prof_tdata_merge_iter, &prof_tdata_merge_iter_arg);
    malloc_mutex_unlock(tsd_tsdn(tsd), &tdatas_mtx);

    /* Merge tctx stats into gctx's. */
    *leak_ngctx = 0;
    prof_gctx_merge_iter_arg_t prof_gctx_merge_iter_arg = {tsd_tsdn(tsd), leak_ngctx};
    gctx_tree_iter(gctxs, NULL, prof_gctx_merge_iter, &prof_gctx_merge_iter_arg);

    prof_leave(tsd, tdata);
}

void prof_dump_impl(tsd_t *tsd, write_cb_t *prof_dump_write, void *cbopaque, prof_tdata_t *tdata, bool leakcheck) {
    malloc_mutex_assert_owner(tsd_tsdn(tsd), &prof_dump_mtx);
    prof_cnt_t cnt_all;
    size_t leak_ngctx;
    prof_gctx_tree_t gctxs;
    prof_dump_prep(tsd, tdata, &cnt_all, &leak_ngctx, &gctxs);
    prof_dump_iter_arg_t prof_dump_iter_arg = {tsd_tsdn(tsd), prof_dump_write, cbopaque};
...
prof_tdata_t *prof_tdata_init_impl(tsd_t *tsd, uint64_t thr_uid, uint64_t thr_discrim, char *thread_name, bool active) {
    assert(tsd_reentrancy_level_get(tsd) == 0);
    ...
    tdata = (prof_tdata_t *)iallocztm(
        tsd_tsdn(tsd), sizeof(prof_tdata_t), sz_size2index(sizeof(prof_tdata_t)), false, NULL, true, arena_get(TSDN_NULL, 0, true), true
    );
    if (tdata == NULL) {
        return NULL;
    }
...
static prof_tdata_t *prof_tdata_reset_iter(prof_tdata_tree_t *tdatas_ptr, prof_tdata_t *tdata, void *arg) {
    tsdn_t *tsdn = (tsdn_t *)arg;
...
static void prof_tctx_destroy(tsd_t *tsd, prof_tctx_t *tctx) {
...

    {
        ...
        bool destroy_tdata = prof_tdata_should_destroy(tsd_tsdn(tsd), tdata, false);
        malloc_mutex_unlock(tsd_tsdn(tsd), tdata->lock);
        if (destroy_tdata) {
