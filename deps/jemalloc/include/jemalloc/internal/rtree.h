...
#elif RTREE_HEIGHT == 2
    {RTREE_NSB / 2, RTREE_NHIB + RTREE_NSB / 2}, {RTREE_NSB / 2 + RTREE_NSB % 2, RTREE_NHIB + RTREE_NSB}
#elif RTREE_HEIGHT == 3
...
JEMALLOC_ALWAYS_INLINE unsigned rtree_leaf_maskbits(void) {
...
JEMALLOC_ALWAYS_INLINE uintptr_t rtree_leaf_elm_bits_encode(rtree_contents_t contents) {
...
JEMALLOC_ALWAYS_INLINE rtree_contents_t rtree_leaf_elm_bits_decode(uintptr_t bits) {
    ...
    contents.edata = (edata_t *)((uintptr_t)((intptr_t)(bits << RTREE_NHIB) >> RTREE_NHIB) & low_bit_mask);
#endif
    assert((uintptr_t)contents.edata % (uintptr_t)EDATA_ALIGNMENT == 0);
    return contents;
}
...
JEMALLOC_ALWAYS_INLINE rtree_contents_t rtree_leaf_elm_read(tsdn_t *tsdn, rtree_t *rtree, rtree_leaf_elm_t *elm, bool dependent) {
#ifdef RTREE_LEAF_COMPACT
...
#else
    rtree_contents_t contents;
    unsigned metadata_bits = atomic_load_u(&elm->le_metadata, dependent ? ATOMIC_RELAXED : ATOMIC_ACQUIRE);
    ...
    contents.edata = (edata_t *)atomic_load_p(&elm->le_edata, dependent ? ATOMIC_RELAXED : ATOMIC_ACQUIRE);

    return contents;
#endif
}

JEMALLOC_ALWAYS_INLINE void rtree_contents_encode(rtree_contents_t contents, void **bits, unsigned *additional) {
#ifdef RTREE_LEAF_COMPACT
    *bits = (void *)rtree_leaf_elm_bits_encode(contents);
#else
    *additional = (unsigned)contents.metadata.slab | ((unsigned)contents.metadata.is_head << 1) |
...
JEMALLOC_ALWAYS_INLINE void rtree_leaf_elm_write_commit(tsdn_t *tsdn, rtree_t *rtree, rtree_leaf_elm_t *elm, void *bits, unsigned additional) {
#ifdef RTREE_LEAF_COMPACT
...
JEMALLOC_ALWAYS_INLINE void rtree_leaf_elm_write(tsdn_t *tsdn, rtree_t *rtree, rtree_leaf_elm_t *elm, rtree_contents_t contents) {
...
JEMALLOC_ALWAYS_INLINE void rtree_leaf_elm_state_update(
    tsdn_t *tsdn, rtree_t *rtree, rtree_leaf_elm_t *elm1, rtree_leaf_elm_t *elm2, extent_state_t state
) {
    assert(elm1 != NULL);
#ifdef RTREE_LEAF_COMPACT
    uintptr_t bits = rtree_leaf_elm_bits_read(
        tsdn, rtree, elm1,
        /* dependent */ true
    );
 ...
 */
JEMALLOC_ALWAYS_INLINE bool rtree_leaf_elm_lookup_fast(tsdn_t *tsdn, rtree_t *rtree, rtree_ctx_t *rtree_ctx, uintptr_t key, rtree_leaf_elm_t **elm) {
...
JEMALLOC_ALWAYS_INLINE rtree_leaf_elm_t *rtree_leaf_elm_lookup(
    tsdn_t *tsdn, rtree_t *rtree, rtree_ctx_t *rtree_ctx, uintptr_t key, bool dependent, bool init_missing
) {
    assert(key != 0);
 ...
 */
static inline bool rtree_read_independent(tsdn_t *tsdn, rtree_t *rtree, rtree_ctx_t *rtree_ctx, uintptr_t key, rtree_contents_t *r_contents) {
    rtree_leaf_elm_t *elm = rtree_leaf_elm_lookup(tsdn, rtree, rtree_ctx, key, /* dependent */ false, /* init_missing */ false);
    if (elm == NULL) {
        return true;
    }
    *r_contents = rtree_leaf_elm_read(
        tsdn, rtree, elm,
        /* dependent */ false
    );
    return false;
...
static inline rtree_contents_t rtree_read(tsdn_t *tsdn, rtree_t *rtree, rtree_ctx_t *rtree_ctx, uintptr_t key) {
    rtree_leaf_elm_t *elm = rtree_leaf_elm_lookup(tsdn, rtree, rtree_ctx, key, /* dependent */ true, /* init_missing */ false);
    assert(elm != NULL);
    return rtree_leaf_elm_read(tsdn, rtree, elm, /* dependent */ true);
}

static inline rtree_metadata_t rtree_metadata_read(tsdn_t *tsdn, rtree_t *rtree, rtree_ctx_t *rtree_ctx, uintptr_t key) {
    rtree_leaf_elm_t *elm = rtree_leaf_elm_lookup(tsdn, rtree, rtree_ctx, key, /* dependent */ true, /* init_missing */ false);
    assert(elm != NULL);
    return rtree_leaf_elm_read(
               tsdn, rtree, elm,
               /* dependent */ true
    )
        .metadata;
 ...
 */
static inline bool rtree_metadata_try_read_fast(
    tsdn_t *tsdn, rtree_t *rtree, rtree_ctx_t *rtree_ctx, uintptr_t key, rtree_metadata_t *r_rtree_metadata
) {
    ...
    assert(elm != NULL);
    *r_rtree_metadata = rtree_leaf_elm_read(
                            tsdn, rtree, elm,
                            /* dependent */ true
    )
                            .metadata;
...
JEMALLOC_ALWAYS_INLINE void rtree_write_range_impl(
    tsdn_t *tsdn, rtree_t *rtree, rtree_ctx_t *rtree_ctx, uintptr_t base, uintptr_t end, rtree_contents_t contents, bool clearing
) {
    ...
    for (uintptr_t addr = base; addr <= end; addr += PAGE) {
        if (addr == base || (addr & ((ZU(1) << rtree_leaf_maskbits()) - 1)) == 0) {
            elm = rtree_leaf_elm_lookup(
                tsdn, rtree, rtree_ctx, addr,
...
JEMALLOC_ALWAYS_INLINE void rtree_write_range(
    tsdn_t *tsdn, rtree_t *rtree, rtree_ctx_t *rtree_ctx, uintptr_t base, uintptr_t end, rtree_contents_t contents
) {
    rtree_write_range_impl(
        tsdn, rtree, rtree_ctx, base, end, contents,
        /* clearing */ false
    );
}

JEMALLOC_ALWAYS_INLINE bool rtree_write(tsdn_t *tsdn, rtree_t *rtree, rtree_ctx_t *rtree_ctx, uintptr_t key, rtree_contents_t contents) {
    rtree_leaf_elm_t *elm = rtree_leaf_elm_lookup(tsdn, rtree, rtree_ctx, key, /* dependent */ false, /* init_missing */ true);
    if (elm == NULL) {
        return true;
    }
...
static inline void rtree_clear_range(tsdn_t *tsdn, rtree_t *rtree, rtree_ctx_t *rtree_ctx, uintptr_t base, uintptr_t end) {
    rtree_contents_t contents;
    contents.edata = NULL;
    contents.metadata.szind = SC_NSIZES;
    contents.metadata.slab = false;
