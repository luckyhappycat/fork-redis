...
static inline bool pa_shard_dont_decay_muzzy(pa_shard_t *shard) {
    return ecache_npages_get(&shard->pac.ecache_muzzy) == 0 && pac_decay_ms_get(&shard->pac, extent_state_muzzy) <= 0;
}

static inline ehooks_t *pa_shard_ehooks_get(pa_shard_t *shard) {
...
bool pa_shard_init(
    ...
    malloc_mutex_t *stats_mtx, nstime_t *cur_time, size_t oversize_threshold, ssize_t dirty_decay_ms, ssize_t muzzy_decay_ms
);
