...
JEMALLOC_ALWAYS_INLINE bool counter_accum(tsdn_t *tsdn, counter_accum_t *counter, uint64_t bytes) {
     ...
     * allocation.
     */
    bool overflow = locked_inc_mod_u64(tsdn, LOCKEDINT_MTX(counter->mtx), &counter->accumbytes, bytes, interval);
