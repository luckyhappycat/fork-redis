...
static inline uint64_t peak_max(peak_t *peak) {
    return peak->cur_max;
}

static inline void peak_update(peak_t *peak, uint64_t alloc, uint64_t dalloc) {
...
static inline void peak_set_zero(peak_t *peak, uint64_t alloc, uint64_t dalloc) {
    peak->cur_max = 0;
