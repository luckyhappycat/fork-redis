...
JEMALLOC_ALWAYS_INLINE bool tcache_small_bin_disabled(szind_t ind, cache_bin_t *bin) {
    assert(ind < SC_NBINS);
...
JEMALLOC_ALWAYS_INLINE void *tcache_alloc_small(
    tsd_t *tsd, arena_t *arena, tcache_t *tcache, size_t size, szind_t binind, bool zero, bool slow_path
) {
    void *ret;
    bool tcache_success;
...
JEMALLOC_ALWAYS_INLINE void *tcache_alloc_large(
    tsd_t *tsd, arena_t *arena, tcache_t *tcache, size_t size, szind_t binind, bool zero, bool slow_path
) {
    void *ret;
    bool tcache_success;
...
JEMALLOC_ALWAYS_INLINE void tcache_dalloc_small(tsd_t *tsd, tcache_t *tcache, void *ptr, szind_t binind, bool slow_path) {
    ...
    if (unlikely(!cache_bin_dalloc_easy(bin, ptr))) {
        ...
        cache_bin_sz_t max = cache_bin_info_ncached_max(&tcache_bin_info[binind]);
...
JEMALLOC_ALWAYS_INLINE void tcache_dalloc_large(tsd_t *tsd, tcache_t *tcache, void *ptr, szind_t binind, bool slow_path) {
    ...
    cache_bin_t *bin = &tcache->bins[binind];
    if (unlikely(!cache_bin_dalloc_easy(bin, ptr))) {
        unsigned remain = cache_bin_info_ncached_max(&tcache_bin_info[binind]) >> opt_lg_tcache_flush_large_div;
