...
#define LOCKEDINT_MTX_INIT(mu, name, rank, rank_mode) malloc_mutex_init(&(mu), name, rank, rank_mode)
...
#define LOCKEDINT_MTX_POSTFORK_PARENT(tsdn, mu) malloc_mutex_postfork_parent(tsdn, &(mu))
#define LOCKEDINT_MTX_POSTFORK_CHILD(tsdn, mu) malloc_mutex_postfork_child(tsdn, &(mu))
#else
...
#else
#define LOCKEDINT_MTX_ASSERT_INTERNAL(tsdn, mtx) malloc_mutex_assert_owner(tsdn, (mtx))
#endif

static inline uint64_t locked_read_u64(tsdn_t *tsdn, malloc_mutex_t *mtx, locked_u64_t *p) {
...
static inline void locked_inc_u64(tsdn_t *tsdn, malloc_mutex_t *mtx, locked_u64_t *p, uint64_t x) {
    LOCKEDINT_MTX_ASSERT_INTERNAL(tsdn, mtx);
#ifdef JEMALLOC_ATOMIC_U64
...
static inline void locked_dec_u64(tsdn_t *tsdn, malloc_mutex_t *mtx, locked_u64_t *p, uint64_t x) {
    LOCKEDINT_MTX_ASSERT_INTERNAL(tsdn, mtx);
#ifdef JEMALLOC_ATOMIC_U64
...
static inline bool locked_inc_mod_u64(tsdn_t *tsdn, malloc_mutex_t *mtx, locked_u64_t *p, const uint64_t x, const uint64_t modulus) {
    LOCKEDINT_MTX_ASSERT_INTERNAL(tsdn, mtx);
    ...
    } while (!atomic_compare_exchange_weak_u64(&p->val, &before, after, ATOMIC_RELAXED, ATOMIC_RELAXED));
#else
    before = p->val;
    after = before + x;
    overflow = (after >= modulus);
 ...
 */
static inline void locked_inc_u64_unsynchronized(locked_u64_t *dst, uint64_t src) {
#ifdef JEMALLOC_ATOMIC_U64
...
static inline uint64_t locked_read_u64_unsynchronized(locked_u64_t *p) {
#ifdef JEMALLOC_ATOMIC_U64
    return atomic_load_u64(&p->val, ATOMIC_RELAXED);
#else
    return p->val;
...
static inline void locked_init_u64_unsynchronized(locked_u64_t *p, uint64_t x) {
#ifdef JEMALLOC_ATOMIC_U64
...
static inline size_t locked_read_zu(tsdn_t *tsdn, malloc_mutex_t *mtx, locked_zu_t *p) {
    LOCKEDINT_MTX_ASSERT_INTERNAL(tsdn, mtx);
#ifdef JEMALLOC_ATOMIC_U64
...
static inline void locked_inc_zu(tsdn_t *tsdn, malloc_mutex_t *mtx, locked_zu_t *p, size_t x) {
    LOCKEDINT_MTX_ASSERT_INTERNAL(tsdn, mtx);
#ifdef JEMALLOC_ATOMIC_U64
...
static inline void locked_dec_zu(tsdn_t *tsdn, malloc_mutex_t *mtx, locked_zu_t *p, size_t x) {
    LOCKEDINT_MTX_ASSERT_INTERNAL(tsdn, mtx);
#ifdef JEMALLOC_ATOMIC_U64
...
static inline void locked_inc_zu_unsynchronized(locked_zu_t *dst, size_t src) {
 ...
 */
static inline size_t locked_read_atomic_zu(locked_zu_t *p) {
    return atomic_load_zu(&p->val, ATOMIC_RELAXED);
}
