 ...
 * from it.
 */
static const uintptr_t cache_bin_preceding_junk = (uintptr_t)0x7a7a7a7a7a7a7a7aULL;
/* Note: a7 vs. 7a above -- this tells you which pointer leaked. */
static const uintptr_t cache_bin_trailing_junk = (uintptr_t)0xa7a7a7a7a7a7a7a7ULL;
 ...
 */
#define CACHE_BIN_NCACHED_MAX (((size_t)1 << sizeof(cache_bin_sz_t) * 8) / sizeof(void *) - 1)
...
static inline void cache_bin_array_descriptor_init(cache_bin_array_descriptor_t *descriptor, cache_bin_t *bins) {
    ql_elm_new(descriptor, link);
    descriptor->bins = bins;
}

JEMALLOC_ALWAYS_INLINE bool cache_bin_nonfast_aligned(const void *ptr) {
...
static inline cache_bin_sz_t cache_bin_info_ncached_max(cache_bin_info_t *info) {
 ...
 * with later.
 */
static inline void cache_bin_assert_earlier(cache_bin_t *bin, uint16_t earlier, uint16_t later) {
    if (earlier > later) {
 ...
 */
static inline uint16_t cache_bin_diff(cache_bin_t *bin, uint16_t earlier, uint16_t later, bool racy) {
    /*
 ...
 */
static inline cache_bin_sz_t cache_bin_ncached_get_internal(cache_bin_t *bin, bool racy) {
    cache_bin_sz_t diff = cache_bin_diff(bin, (uint16_t)(uintptr_t)bin->stack_head, bin->low_bits_empty, racy);
 ...
 * possible.
 */
static inline cache_bin_sz_t cache_bin_ncached_get_local(cache_bin_t *bin, cache_bin_info_t *info) {
    cache_bin_sz_t n = cache_bin_ncached_get_internal(
        bin,
        /* racy */ false
    );
 ...
 */
static inline void **cache_bin_empty_position_get(cache_bin_t *bin) {
    cache_bin_sz_t diff = cache_bin_diff(
        bin, (uint16_t)(uintptr_t)bin->stack_head, bin->low_bits_empty,
        /* racy */ false
    );
 ...
 */
static inline uint16_t cache_bin_low_bits_low_bound_get(cache_bin_t *bin, cache_bin_info_t *info) {
 ...
 */
static inline void **cache_bin_low_bound_get(cache_bin_t *bin, cache_bin_info_t *info) {
 ...
 */
static inline void cache_bin_assert_empty(cache_bin_t *bin, cache_bin_info_t *info) {
 ...
 */
static inline cache_bin_sz_t cache_bin_low_water_get_internal(cache_bin_t *bin) {
    return cache_bin_diff(bin, bin->low_bits_low_water, bin->low_bits_empty, /* racy */ false) / sizeof(void *);
}

/* Returns the numeric value of low water in [0, ncached]. */
static inline cache_bin_sz_t cache_bin_low_water_get(cache_bin_t *bin, cache_bin_info_t *info) {
    ...
    cache_bin_assert_earlier(bin, (uint16_t)(uintptr_t)bin->stack_head, bin->low_bits_low_water);
 ...
 * going forward.
 */
static inline void cache_bin_low_water_set(cache_bin_t *bin) {
    bin->low_bits_low_water = (uint16_t)(uintptr_t)bin->stack_head;
}

static inline void cache_bin_low_water_adjust(cache_bin_t *bin) {
    if (cache_bin_ncached_get_internal(bin, /* racy */ false) < cache_bin_low_water_get_internal(bin)) {
        cache_bin_low_water_set(bin);
    }
}

JEMALLOC_ALWAYS_INLINE void *cache_bin_alloc_impl(cache_bin_t *bin, bool *success, bool adjust_low_water) {
    /*
 ...
 */
JEMALLOC_ALWAYS_INLINE void *cache_bin_alloc(cache_bin_t *bin, bool *success) {
    return cache_bin_alloc_impl(bin, success, true);
}

JEMALLOC_ALWAYS_INLINE cache_bin_sz_t cache_bin_alloc_batch(cache_bin_t *bin, size_t num, void **out) {
    cache_bin_sz_t n = cache_bin_ncached_get_internal(
        bin,
        /* racy */ false
    );
...
JEMALLOC_ALWAYS_INLINE bool cache_bin_full(cache_bin_t *bin) {
 ...
 */
JEMALLOC_ALWAYS_INLINE bool cache_bin_dalloc_easy(cache_bin_t *bin, void *ptr) {
...
JEMALLOC_ALWAYS_INLINE bool cache_bin_stash(cache_bin_t *bin, void *ptr) {
 ...
 * made.
 */
JEMALLOC_ALWAYS_INLINE cache_bin_sz_t cache_bin_nstashed_get_internal(cache_bin_t *bin, cache_bin_info_t *info, bool racy) {
    cache_bin_sz_t ncached_max = cache_bin_info_ncached_max(info);
    uint16_t low_bits_low_bound = cache_bin_low_bits_low_bound_get(bin, info);

    cache_bin_sz_t n = cache_bin_diff(bin, low_bits_low_bound, bin->low_bits_full, racy) / sizeof(void *);
...
JEMALLOC_ALWAYS_INLINE cache_bin_sz_t cache_bin_nstashed_get_local(cache_bin_t *bin, cache_bin_info_t *info) {
    cache_bin_sz_t n = cache_bin_nstashed_get_internal(
        bin, info,
        /* racy */ false
    );
    assert(n <= cache_bin_info_ncached_max(info));
 ...
 */
static inline void cache_bin_nitems_get_remote(cache_bin_t *bin, cache_bin_info_t *info, cache_bin_sz_t *ncached, cache_bin_sz_t *nstashed) {
 ...
 */
static inline void cache_bin_init_ptr_array_for_fill(cache_bin_t *bin, cache_bin_info_t *info, cache_bin_ptr_array_t *arr, cache_bin_sz_t nfill) {
 ...
 * case of OOM.
 */
static inline void cache_bin_finish_fill(cache_bin_t *bin, cache_bin_info_t *info, cache_bin_ptr_array_t *arr, cache_bin_sz_t nfilled) {
    cache_bin_assert_empty(bin, info);
 ...
 */
static inline void cache_bin_init_ptr_array_for_flush(cache_bin_t *bin, cache_bin_info_t *info, cache_bin_ptr_array_t *arr, cache_bin_sz_t nflush) {
    arr->ptr = cache_bin_empty_position_get(bin) - nflush;
    assert(cache_bin_ncached_get_local(bin, info) == 0 || *arr->ptr != NULL);
}

static inline void cache_bin_finish_flush(cache_bin_t *bin, cache_bin_info_t *info, cache_bin_ptr_array_t *arr, cache_bin_sz_t nflushed) {
...
static inline void cache_bin_init_ptr_array_for_stashed(
    cache_bin_t *bin, szind_t binind, cache_bin_info_t *info, cache_bin_ptr_array_t *arr, cache_bin_sz_t nstashed
) {
    assert(nstashed > 0);
...
static inline void cache_bin_finish_flush_stashed(cache_bin_t *bin, cache_bin_info_t *info) {
    void **low_bound = cache_bin_low_bound_get(bin, info);
 ...
 */
void cache_bin_info_init(cache_bin_info_t *bin_info, cache_bin_sz_t ncached_max);
/*
 ...
 */
void cache_bin_info_compute_alloc(cache_bin_info_t *infos, szind_t ninfos, size_t *size, size_t *alignment);
 ...
 * of the allocation.
 */
void cache_bin_preincrement(cache_bin_info_t *infos, szind_t ninfos, void *alloc, size_t *cur_offset);
void cache_bin_postincrement(cache_bin_info_t *infos, szind_t ninfos, void *alloc, size_t *cur_offset);
void cache_bin_init(cache_bin_t *bin, cache_bin_info_t *info, void *alloc, size_t *cur_offset);
